...
const ARTIFACT_DIRECTORY = path.join(tmpdir(), "zettlr-review-diff-save-gate-e2e-latest");
...
function agentClient(port: number): AgentClient {
  ...
  const readResponse = async (response: Response): Promise<unknown> => {
    ...
    if (!response.ok) {
      throw new Error(`Agent API ${response.status} for ${response.url}: ${text}`);
    }
...
async function waitForAgentApi(client: AgentClient, timeoutMs: number): Promise<void> {
...
async function reviewCounts(
  ...
  assert.ok(
    detail !== null &&
      ...
      "unresolvedChunks" in detail &&
      typeof detail.unresolvedChunks === "number" &&
...
async function openReview(
  ...
  const reviewId = reviewIdOf(
    await client.post(`/v1/documents/${documentId}/proposals`, {
      baselineSha256,
 ...
 */
async function clearReviewAndFlush(page: Page, documentPath: string): Promise<void> {
...
async function acceptEveryChunk(page: Page, timeoutMs: number): Promise<number> {
  ...
  assert.equal(await accept.count(), 0, "Review controls remained after accepting every chunk.");
  assert.ok(accepted > 0, "The review pane rendered no accept control, so nothing was proven.");
  return accepted;
}
...
describe("saving after accepting a reviewed change", function () {
  ...
  it("writes the accepted text to disk instead of refusing the save", async function () {
    const activeClient = requireInitialized(
      running.client,
      "The Agent API client must be initialized",
    );
    ...
    await activeClient.post(`/v1/documents/${documentId}/proposals`, {
      baselineSha256,
      expectedReviewGeneration: 0,
  ...
  it("keeps a chunk comment rendered and agent-readable after saving", async function () {
    ...
    await chunkWidget.locator("input.cm-chunkCommentInput").fill("Preserve this note across save");
    const agentSeesNote = async (): Promise<boolean> => {
      ...
      return (
        (noted as { comment?: unknown } | undefined)?.comment === "Preserve this note across save"
      );
    };
    ...
    assert.ok(
      afterChunks !== null &&
        typeof afterChunks === "object" &&
        "chunks" in afterChunks &&
        Array.isArray(afterChunks.chunks),
      `The chunk listing must survive the save: ${JSON.stringify(afterChunks)}`,
    );
    const annotated = (afterChunks.chunks as Array<{ chunkId: string; comment?: unknown }>).find(
      (chunk) => chunk.chunkId === notedChunkId,
    );
    assert.ok(
      annotated !== undefined,
  ...
  it("saves a pending review as-is and keeps it open", async function () {
    ...
    assert.equal(
      after.generation,
      before.generation,
      "A save must not advance the review generation.",
    );
    await page
      .locator("button.cm-review-diff-control.accept")
      .first()
  ...
  it("reattaches the saved pending review when the document reopens", async function () {
    ...
    const savedReviewId = requireInitialized(
      pendingReviewId,
      "The previous test must have saved a pending review",
    );
    assert.ok(running.browser, "The application must be running");
    const page = await findEditorPage(running.browser, this.timeout());
