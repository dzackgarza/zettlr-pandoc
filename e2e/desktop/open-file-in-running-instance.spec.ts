...
const ARTIFACT_DIRECTORY = path.join(
  tmpdir(),
  'zettlr-open-in-running-e2e-latest'
)
const HANDOFF_SCRIPT = path.join(
  REPO_ROOT,
  'scripts',
  'desktop',
  'zettlr-pandoc-open-in-running'
)
...
describe('opening a file while the app is already running', function () {
  ...
  before(async function () {
    ...
    // handoff.
    deliveredDocumentPath = path.join(
      path.dirname(fixture.documentPath),
      'delivered.md'
    )
  ...
  it('gains a tab for the delivered file without restarting the app', async function () {
    ...
    const dataDirectory = requireInitialized(
      configDirectory,
    ...
    const runningProcess = requireInitialized(
      appProcess,
      'The application process must exist'
    )
    ...
    const deliveredTab = editorPage.locator(
      `div[role="tab"][data-path="${deliveredPath}"]`
    )
    ...
    assert.equal(
      await editorPage
        .locator(`div[role="tab"][data-path="${bootDocumentPath}"]`)
        .count(),
      1,
  ...
  it('reports a failure rather than opening a second app when no instance holds the lock', async function () {
    ...
    assert.notEqual(
      handoff.code,
      0,
      `An undelivered file must fail loudly.\n${handoff.output}`
    )
