...
const ARTIFACT_DIRECTORY = path.join(tmpdir(), "zettlr-document-open-e2e-latest");
...
async function waitForAppDiagnostic(
  ...
  throw new Error(`No app diagnostic identified ${documentPath} and EACCES within ${timeoutMs}ms.`);
}
...
describe("opening a Markdown document", function () {
  ...
  it("renders reference UI and opens a badge citing location on first load", async function () {
    ...
    const page = await findEditorPage(browser, this.timeout());
    const countBadge = page.locator('.reference-count-badge[data-reference-key="sec:terminology"]');
    ...
    assert.equal(
      ...
      `The real editor did not render the expected reference count badge.\n` +
        ...
        `Rendered badge keys: ${JSON.stringify(
          await page
            .locator(".reference-count-badge")
            .evaluateAll((badges) =>
              badges.map((badge) => (badge as HTMLElement).dataset.referenceKey),
            ),
        )}`,
    );
    ...
    const overlay = page.locator('.reference-search-overlay[data-search-mode="citing-locations"]');
    ...
    assert.equal(await locations.first().getAttribute("data-occurrence-path"), activeDocumentPath);
    assert.match(await locations.first().innerText(), /@sec:terminology/);
    screenshots.set("reference-badge-citing-location.png", await page.screenshot());
  ...
  it("attributes a remote reload failure to the active document", async function () {
    ...
    assert.ok(
      toastText.includes(path.basename(activeDocumentPath)) && toastText.includes("EACCES"),
    ...
    const matchingDiagnostics = rendererEvents.filter(
      (event) => event.includes(activeDocumentPath) && event.includes("EACCES"),
    );
    assert.equal(
