...
interface CSLKernelConfig {
  ...
  /**
   * An optional locale-aware comparator citeproc sorts with, in the shape
   * Array.prototype.sort expects. Zettlr supplies none, so the engine falls
   * back to its own ordering.
   */
  stringCompare?: (left: string, right: string) => number;
}
...
type ProcessCitationClusterResult = [
  ...
  metadata: {
    bibchange: boolean;
    /** What the engine could not resolve while processing, as messages. */
