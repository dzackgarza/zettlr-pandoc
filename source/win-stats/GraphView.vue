...
const buildProgress = ref<{ currentFile: number; totalFiles: number }>({
  currentFile: 0,
  totalFiles: 0,
});
...
watch(highlightFilter, () => {
  ...
  const matches = graphElement.value
    .selectAll("#vertex-container g")
    .filter((datum: any) => {
  ...
  const nonmatches = graphElement.value
    .selectAll("#vertex-container g")
    .filter((datum: any) => {
      return !(datum.id as string).toLowerCase().includes(query);
    })
...
onMounted(() => {
  ...
  graphElement.value.call(d3.zoom<SVGSVGElement, any>()).on("wheel.zoom", (event: WheelEvent) => {
...
function startSimulation(): void {
  ...
  if (simulation.value === null) {
    const forceLink = d3
      .forceLink<GraphVertex & SimulationNodeDatum, GraphArc>(includedLinks)
      .id((node, _i, _nodesData) => node.id)
      .strength((link, _i) => link.weight * 2);
    ...
    simulation.value = d3
      .forceSimulation(includedNodes as any)
    ...
    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
    simulation.value
      .nodes(includedNodes as any)
      .alpha(1)
      .alphaTarget(0)
      .restart();
    const link: d3.ForceLink<GraphVertex & d3.SimulationNodeDatum, GraphArc> =
  ...
  const linkSelection = svg.select("#arc-container").selectAll("line").data(includedLinks);
    ...
    .join(
      (enter) => {
        ...
        groupSelection
          .append("circle")
          .attr("r", 5)
          .attr("fill", (vertex, _value) =>
            vertex.isolate ? color(ISOLATES_CLASS) : color(vertex.component),
          )
...
async function buildGraph(): Promise<void> {
  ...
  const dbObject: Record<string, string[]> = await ipcRenderer.invoke("link-provider", {
  ...
  for (const [sourcePath, targets] of database) {
    ...
    const sourceDescriptor: MDFileDescriptor | undefined = await ipcRenderer.invoke("fsal", {
    ...
    for (const target of targets) {
      ...
      if (!resolvedLinks.has(target)) {
        const found: MDFileDescriptor | undefined = await ipcRenderer.invoke("application", {
          command: "find-exact",
          payload: target,
        });
        if (found === undefined) {
  ...
  console.log(
    `[Link Provider] Graph constructed in ${Math.round(duration)}ms. Graph contains ${DG.countVertices} nodes, ${DG.countArcs} arcs and ${DG.countComponents} components.`,
  );
