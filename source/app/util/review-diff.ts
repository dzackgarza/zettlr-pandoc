...
export function buildReviewDiffSessionFromBaseline(
  request: ReviewDiffSessionFromBaselineRequest,
): ReviewDiffSession {
...
function buildProposedText(
  ...
  const patchText =
    request.patchText ??
    (request.patchPath !== undefined
      ? fs.readFileSync(request.patchPath, { encoding: "utf8" })
      : undefined);
...
function getSingleSupportedPatch(
  ...
  if (
    !patchHeaderMatchesDocument(patch.oldFileName, documentPath) ||
    !patchHeaderMatchesDocument(patch.newFileName, documentPath)
  ) {
