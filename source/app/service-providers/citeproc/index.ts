...
export type CiteprocProviderIPCAPI =
  ...
  | {
      ...
      payload: { database: string; citations: CiteItem[]; composite: boolean };
    };
...
export default class CiteprocProvider extends ProviderContract {
  ...
  public getBibTexAttachments(dbPath: string, id: string): string[] | false {
    ...
    const attachments = db.bibtexAttachments[id];
  ...
  /**
   * The "mode: composite" rendering of processCitationCluster, mimicked for a
   * single item: the author rendered on its own, then the rest of the citation
   * with the author suppressed. The item's flags are restored between the two
   * clusters because the engine reads them off the item it was handed.
   *
   * @param  {CiteItem}  citation  The single cited item.
   *
   * @return {string}              Author, then the remainder of the citation.
   */
  private makeCompositeCluster(citation: CiteItem): string {
  ...
  getCitation(
    ...
    try {
      ...
      if (composite && citations.length === 1) {
        return this.makeCompositeCluster(citations[0]);
      }
      return this.engine.makeCitationCluster(citations);
    } catch (err: unknown) {
      const msg = citations.map((elem) => elem.id).join(", ");
      this._logger.error(
        `[citeproc] makeCitationCluster: Could not create citation cluster ${msg}: ${err instanceof Error ? err.message : "unknown error"}`,
        err,
      );
      return undefined;
    }
  ...
  makeBibliography(
    ...
    try {
      ...
      this._logger.error(
        `[citeproc] makeBibliography: Could not create bibliography: ${err instanceof Error ? err.message : "unknown error"}`,
        err,
      );
