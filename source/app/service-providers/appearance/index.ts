...
export default class AppearanceProvider extends ProviderContract {
    ...
    if (process.platform === "darwin") {
      ...
      systemPreferences.subscribeNotification("AppleInterfaceThemeChangedNotification", (event) => {
        ...
        if (this._mode === "system" && nativeTheme.themeSource !== "system") {
          this._logger.info(
            "[Appearance Provider] Received an AppleInterfaceThemeChangedNotification, and detected that the themeSource was forcefully overridden. Switching back to system.",
          );
  ...
  private recalculateSchedule(): void {
    ...
    if (this._startHour === this._endHour && this._startMin === this._endMin) {
      this._endMin++;
    }
  }
  ...
  private scheduleIsDark(): boolean {
    ...
    const isOvernight =
      ...
      (this._startHour === this._endHour && this._startMin > this._endMin);
    const nowLaterThanStart =
      nowHours > this._startHour || (nowHours === this._startHour && nowMin >= this._startMin);
    const nowEarlierThanEnd =
      nowHours < this._endHour || (nowHours === this._endHour && nowMin < this._endMin);
