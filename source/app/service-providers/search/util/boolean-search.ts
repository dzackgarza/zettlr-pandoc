 ...
 */
export function searchFileBoolean(
  ...
  for (const t of termsToSearch) {
    ...
    for (const wd of t.words) {
      if (fileName.includes(wd) || (descriptor.type === "file" && descriptor.tags.includes(wd))) {
        ...
        descriptor.tags.includes(wd.substring(1))
      ) {
  ...
  for (const t of termsToSearch) {
    ...
    for (const wd of t.words) {
      const newMatches: SearchResult = findAll(wd, content)
        ...
        .reduce<Array<{ line: number; ranges: Array<{ from: number; to: number }> }>>(
          (prev, cur) => {
 ...
 */
function getTextSurrounding(
  ranges: Array<{ from: number; to: number }>,
  text: string,
  window: number = 40,
): { relativeRanges: Array<{ from: number; to: number }>; excerpt: string } {
