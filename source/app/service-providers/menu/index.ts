...
export default class MenuProvider extends ProviderContract {
  ...
  constructor(
    ...
    this._config.on("update", () => {
      this.set();
    });
    if (!["darwin", "win32"].includes(process.platform)) {
      this._recentDocs.on("update", () => {
        this.set();
      });
    }
    ...
    ipcMain.on("menu-provider", (event, message) => {
      ...
      if (command === "get-application-menu") {
        ...
        if (appMenu === null) {
          this._logger.error(
            `[Menu Provider] Could not trigger a click on item ${itemID}: No menu set.`,
          );
          return;
        }
        ...
        if (menuItem === null) {
          this._logger.error(
            `[Menu Provider] Could not trigger a click on item ${itemID}: No item found.`,
          );
          return;
        }
        ...
        if (typeof menuItem.role === "string") {
          if (focusedWindow === null) {
            this._logger.error(
              `[Menu Provider] Could not trigger custom click on menuItem ${itemID} with role ${menuItem.role}: No focused Window to trigger on.`,
            );
            return;
          }
          ...
          switch (menuItem.role.toLowerCase()) {
            ...
            default:
              this._logger.error(
                `[Menu Provider] Could not click menu item with role ${menuItem.role}, since no handler is implemented!`,
              );
          }
        } else {
   ...
   */
  private async _displayNativeContextMenu(
