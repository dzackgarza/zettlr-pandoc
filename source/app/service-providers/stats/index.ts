  ...
  async boot(): Promise<void> {
    ...
    try {
        ...
        for (const [key, value] of Object.entries(this.stats[prop])) {
          if (!/^\d{4}-\d{2}-\d{2}$/.test(key)) {
            ...
            this._logger.warning(
              `[Stats Provider] Removed invalid date ${key} from ${prop} with a count of ${value}.`,
            );
      ...
      if (err instanceof Error) {
        this._logger.error(
          `[StatsProvider] Could not read stats file: ${err.message}. Resetting stats to default and moving malformed file to backup: ${path.basename(newPath)}. Please check the file for what caused the error and possibly report this as a potential bug.`,
          err,
        );
      }
    }
