...
export default class LinkProvider extends ProviderContract {
  ...
  public async boot(): Promise<void> {
    ...
    this._fsal.on("fsal-event", () => {
      ...
      this.reindex().catch((err) =>
        this._logger.error(
          `[LinkProvider] Could not update link and ID database: ${err.message}`,
          err,
        ),
      );
  ...
  private async findExact(
    query: string,
    descriptors: MDFileDescriptor[],
  ): Promise<MDFileDescriptor | undefined> {
  ...
  async retrieveOutbound(sourceFilePath: string): Promise<string[]> {
    ...
    const loadedDescriptors = (await this._fsal.getAllLoadedDescriptors()).filter(
      (descriptor) => descriptor.type === "file",
    );
