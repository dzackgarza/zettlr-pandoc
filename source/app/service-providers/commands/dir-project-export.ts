  ...
  async run(evt: string, arg: string): Promise<boolean> {
    ...
    if (existingFilesWithSorting.length === 0) {
      ...
      dialog.showErrorBox(
        ...
        trans(
          "There are no files selected for export. Please select files to be included in the project settings.",
        ),
      );
      ...
      await dialog.showMessageBox({
        ...
        message: trans(
          "One or more files specified in the project settings have not been found. They may have moved or been deleted. Please verify that all files that you would like to export are included.",
        ),
        buttons: [trans("Ok")],
    ...
    for (const profilePathOrCommand of config.profiles) {
      ...
      const command = this._app.config
        .get()
        .export.customCommands.find((c) => c.command === profilePathOrCommand);

      if (profile === undefined && command === undefined) {
        this._app.log.warning(
          `Could not export project ${dir.name} using profile or command ${profilePathOrCommand}: Not found`,
        );
        continue;
      }
      ...
      if (profile?.isInvalid === true) {
        this._app.log.error(
          `Could not export project ${dir.name} using profile ${profilePathOrCommand}: ${profile.reason}`,
        );
        continue;
      }
 ...
 */
async function exportUsingCustomCommand(
  app: AppServiceContainer,
  dir: DirDescriptor,
  config: ProjectSettings,
  command: { displayName: string; command: string },
) {
  ...
  try {
    ...
    if (!(err instanceof Error)) {
      ...
      task.update({
        ...
        info: trans(
          "Export with command %s was unsuccessful: %s.",
          command.displayName,
          err.message,
        ),
      });
 ...
 */
async function exportUsingProfile(
  app: AppServiceContainer,
  dir: DirDescriptor,
  config: ProjectSettings,
  ...
  const opt: ExporterOptions = {
    ...
    defaultsOverride: {
      title: config.title,
      csl:
        typeof config.cslStyle === "string" && config.cslStyle.length > 0
          ? config.cslStyle
          : undefined,
      template,
    },
