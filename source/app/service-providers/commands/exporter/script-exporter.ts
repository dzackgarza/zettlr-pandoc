...
import type { ExporterAPI, ExporterOptions, ExporterOutput } from "./types";
...
export async function runScriptExport(
  ...
  if (baseProfile === undefined) {
    throw new Error(
      `Export script "${script.name}" references unknown base profile "${script.profile}"`,
    );
  }
  if (baseProfile.isInvalid) {
    throw new Error(
      `Export script "${script.name}" references the unusable base profile "${script.profile}": ${baseProfile.reason}`,
    );
  }
  ...
  const scriptOutput = await runShellCommand(
    script.command,
    [`'${intermediate}'`, `'${target}'`],
    cwd,
  );
