...
export default class TutorialOpen extends ZettlrCommand {
  ...
  async run(event: string, _arg: any): Promise<any> {
    ...
    const candidates = availableLanguages
      .map((e) => {
        return { tag: e, path: path.join(tutorialPath, e) };
      })
    ...
    if (await this._app.fsal.pathExists(targetPath)) {
      this._app.log.info(
        `[Application] The directory ${targetPath} already exists. Not overwriting any files.`,
      );
    } else {
    ...
    if (!this._app.config.get().app.openWorkspaces.includes(targetPath)) {
      this._app.log.error(
        "[Application] Could not open tutorial files: Directory has not been added to Configuration",
      );
      return;
    }
    ...
    if (leafId === undefined) {
      this._app.log.error(
        "[Application] Could not find new split leaf ID -- aborting opening tutorial files.",
      );
      return;
    }
