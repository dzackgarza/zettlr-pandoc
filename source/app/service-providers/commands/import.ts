...
export default class ImportFiles extends ZettlrCommand {
  ...
  async run(event: string, _arg: void): Promise<boolean> {
    const dirs = await this._app.windows.askDir(
      trans("Import directory"),
      undefined,
      trans("Select folder"),
      trans("Select import destination"),
    );
    if (dirs.length === 0) {
    ...
    try {
      ...
      this._app.log.info(
        `[Importer] Importing ${fileList.length} files: ${fileList.join(", ")} ...`,
      );
      const ret = await makeImport(
        fileList,
        directory,
        this._app.assets,
      ...
      if (ret.length > 0) {
        ...
        showNativeNotification(
          trans(
            "The following %s files could not be imported: %s",
            ret.length,
            ret
              .map((x) => {
      ...
      if (err instanceof Error) {
        ...
        showNativeNotification(
          trans("Could not import files due to an error: %s", err.message),
          trans("Import failed"),
        );
      } else {
        this._app.log.error("[Importer] Could not import files: Unknown error", err);
        showNativeNotification(
          trans("Could not import files due to an error: %s", trans("Unknown error")),
          trans("Import failed"),
        );
      }
    }
