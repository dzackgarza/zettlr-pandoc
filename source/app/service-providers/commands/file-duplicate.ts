...
export default class FileDuplicate extends ZettlrCommand {
  ...
  async run(
    evt: string,
    arg: { path: string; name?: string; windowId: string; leafId?: string },
  ): Promise<void> {
    ...
    if (
      dir === undefined &&
      openDirectory !== null &&
      (await this._app.fsal.isDir(openDirectory))
    ) {
    ...
    if (arg.name !== undefined) {
      ...
      if (await this._app.fsal.isFile(path.join(dir.path, filename))) {
        ...
        while (
          await this._app.fsal.isFile(
            path.join(dir.path, `Copy (${duplicateNumber}) of ${file.name}`),
          )
        ) {
          duplicateNumber++;
        }
