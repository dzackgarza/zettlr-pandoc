...
export default class FileRename extends ZettlrCommand {
  ...
  async run(evt: string, arg: { path: string; name: string }): Promise<void> {
    ...
    if (parsedOldPath.ext !== parsedNewPath.ext) {
      const validExt = hasAnyRecognizedFileExtension(
        parsedNewPath.ext,
        this._app.config.get().attachmentExtensions,
      );
      ...
      if (validExt) {
        const response = await dialog.showMessageBox({
          ...
          message: trans(
            "Do you want to change the file extension from %s to %s?",
            parsedOldPath.ext,
            parsedNewPath.ext,
          ),
          buttons: [trans("Use %s", parsedNewPath.ext), trans("Keep %s", parsedOldPath.ext)],
          defaultId: 1,
        });
    ...
    try {
      ...
      if (inboundLinks.length > 0) {
        ...
        for (const file of inboundLinks) {
          ...
          if (newContent !== content) {
            ...
            this._app.log.info(
              `[Application] Replaced link to ${oldName} with ${newName} in file ${file}`,
            );
          }
        }
      }
      ...
      this._app.log.error(
        `Error during renaming file: ${err instanceof Error ? err.message : "Unknown error"}`,
        err,
      );
    }
  }
