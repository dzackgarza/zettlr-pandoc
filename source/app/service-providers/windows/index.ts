...
export default class WindowProvider extends ProviderContract {
  ...
  constructor(
    ...
    ipcMain.on("window-controls", (event, message: WindowControlsIPCAPI) => {
      ...
      switch (command) {
        ...
        case "drag-start":
          app
            .getFileIcon(payload.filePath)
            .then((icon) => {
              event.sender.startDrag({ file: payload.filePath, icon });
            })
            .catch((err) =>
              this._logger.error(
                `[Window Manager] Could not fetch icon for path ${String(payload.filePath)}`,
                err,
              ),
            );
          break;
    ...
    ipcMain.handle("request-files", async (event, message: RequestFilesIPCAPI) => {
      ...
      return await this.askFile(message.filters, message.multiSelection, focusedWindow);
    });
  ...
  public maybeShowWindows(): void {
    ...
    if (!this.suppressWindowOpening || !traySupported) {
      ...
      this._logger.info(
        "[Window Manager] Application should start in tray. Not showing main window.",
      );
    }
  }
  ...
  private _hookMainWindow(window: BrowserWindow): void {
    ...
    window.on("close", (event) => {
      ...
      if (key === undefined) {
        this._logger.error(
          "[Window Manager] A main window is being closed, but I could not find its key!",
        );
        return;
      }
    ...
    window.on("closed", () => {
      ...
      if (key === undefined) {
        this._logger.error(
          "[Window Manager] Could not dereference a main window since its key was not found!",
        );
        return;
      } else {
   ...
   */
  private _retrieveWindowPosition(
    stateId: string,
    defaultSize: Electron.Rectangle | null,
  ): WindowPosition {
  ...
  private syncMainWindows(): void {
    if (this.suppressWindowOpening) {
      this._logger.info(
        "[Window Manager] Received request to synchronize windows, but the app is still in tray mode.",
      );
      return;
    }
  ...
  async showPasteImageModal(startPath: string): Promise<PasteModalResult | undefined> {
    return await new Promise((resolve, reject) => {
      ...
      if (firstMainWin === undefined) {
        return reject(
          new Error(
            "[Window Manager] A paste image modal was requested, but there was no main window open.",
          ),
        );
      }
      this._pasteImageModal = createPasteImageModal(
        this._logger,
        this._config,
        firstMainWin,
        startPath,
  ...
  showErrorMessage(title: string, message: string, contents?: string): void {
    ...
    if (firstMainWin === undefined) {
      this._logger.error(
        "[Application] Could not display error message, because the main window was not open!",
        message,
      );
      return;
    }
    ...
    this._errorModal = createErrorModal(
      this._logger,
      this._config,
      firstMainWin,
      title,
