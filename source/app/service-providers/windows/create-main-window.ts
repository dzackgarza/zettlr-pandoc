...
import { BrowserWindow, type BrowserWindowConstructorOptions, dialog } from "electron";
...
export default function createMainWindow(
  ...
  window.loadURL(effectiveUrl.toString()).catch((error: unknown) => {
    ...
    logger.error(`Could not load URL ${MAIN_WINDOW_WEBPACK_ENTRY}: ${diagnostic}`, error);
    dialog.showErrorBox("Could not open window", `Could not open main Window: ${diagnostic}`);
  });
