 ...
 */
export default async function saveFileDialog(
  logger: LogProvider,
  config: ConfigProvider,
  win: BrowserWindow | null,
  fileOrPathName: string,
  ...
  if (!isDir(path.dirname(fileOrPathName))) {
    ...
    // of a fallback. TODO
    logger.warning(
      `Warning: saveFileDialog has been called with a relative path: ${fileOrPathName}. This behavior is deprecated. Please always provide an absolute path.`,
    );
  }
  ...
  let opt: SaveDialogOptions = {
    ...
    defaultPath: isDir(path.dirname(fileOrPathName))
      ? fileOrPathName
      : path.join(startDir, fileOrPathName),
