...
export async function readDirectoryFromDisk (
  ...
  try {
    ...
    const results = await Promise.allSettled(
      childPaths.map(async childPath => await getDescriptor(childPath)
        .catch(err => logger.error(`[FSAL] Error while reading directory ${absPath}: Could not read child ${path.relative(absPath, childPath)}`, err)))
    )
