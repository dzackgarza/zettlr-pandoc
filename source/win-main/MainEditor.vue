...
import { computed, onBeforeUnmount, onMounted, onUpdated, ref, toRef, watch } from "vue";
 ...
 */
function throwOnReviewRefusal(result: { ok: true } | { ok: false; message: string }): void {
  if (!result.ok) {
...
ipcRenderer.on("shortcut", (event, command) => {
  ...
  if (command === "save-file") {
    ...
    if (configStore.config.editor.formatOnSave && isMarkdown.value && currentEditor !== undefined) {
      ...
      editor
        .runFormatter(ipcMarkdownFormatter)
        .then(async (result) => {
    ...
    if (event === DP_EVENTS.FILE_REMOTE_CHANGE_ERROR && context.filePath === props.file.path) {
      ...
      ipcRenderer
        ...
        .then((descriptor) => {
          ...
          const library =
            descriptor.type === "file" ? getBibliographyForDescriptor(descriptor) : undefined;
...
async function getEditorFor(doc: string): Promise<MarkdownEditor> {
  ...
  editor.setReviewActionClient({
    ...
    acceptAll: async (input) => {
      throwOnReviewRefusal(
        await ipcRenderer.invoke("documents:accept-all-review-chunks", {
          reviewId: input.reviewId,
          expectedReviewGeneration: input.expectedReviewGeneration,
          expectedWorkingSha256: await syncedWorkingSha256(editor),
        }),
    ...
    clear: async (input) => {
      throwOnReviewRefusal(
        await ipcRenderer.invoke("documents:clear-review", {
          reviewId: input.reviewId,
          expectedReviewGeneration: input.expectedReviewGeneration,
          expectedWorkingSha256: await syncedWorkingSha256(editor),
        }),
    ...
    comment: async (input) => {
      ...
      throwOnReviewRefusal(
        await ipcRenderer.invoke("documents:add-review-comment", {
          reviewId: input.reviewId,
          text: input.text,
          expectedReviewGeneration: input.expectedReviewGeneration,
        }),
...
function describeRenameRejection(reason: ReferenceRenameRejection): string {
  switch (reason.kind) {
    ...
    case "family-changed":
      return trans(
        'Cannot rename: references keep their family prefix ("%s:" cannot become "%s:").',
        reason.oldFamily,
        reason.newFamily,
      );
    case "collision":
...
async function updateFileDatabase(): Promise<void> {
  ...
  const rawLinks: Record<string, string[]> = await ipcRenderer.invoke("link-provider", {
...
function maybeHighlightSearchResults(): void {
  ...
  for (const res of result.result.filter(
    (res): res is FileContentSearchResult => res.type === "content" && res.line > -1,
  )) {
