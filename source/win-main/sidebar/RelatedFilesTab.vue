...
async function recomputeRelatedFiles(): Promise<void> {
  ...
  const descriptor: MDFileDescriptor | CodeFileDescriptor | OtherFileDescriptor | undefined =
    await ipcRenderer.invoke("fsal", {
  ...
  tagsOnly.sort(
    (a, b) =>
      b.tags.map((tag) => idf[tag]).reduce((p, c) => p + c, 0) -
      a.tags.map((tag) => idf[tag]).reduce((p, c) => p + c, 0),
  );

  relatedFiles.value = [...backlinksAndTags, ...backlinksOnly, ...tagsOnly];
}
...
function getRelatedFileName(filePath: string): string {
  ...
  if (
    useTitle.value &&
    descriptor.frontmatter !== null &&
    typeof descriptor.frontmatter.title === "string"
  ) {
