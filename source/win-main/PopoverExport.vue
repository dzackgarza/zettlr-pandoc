...
const exportButtonLabel = computed(() =>
  isExporting.value ? trans("Exporting…") : trans("Export"),
);
...
const availableFormats = computed(() => {
  ...
  exportableProfiles.value.forEach((elem) => {
    selectOptions[elem.name] = getDisplayText(elem);
  });
...
function doExport(): void {
  ...
  if (customCommand !== undefined) {
    ...
    isExporting.value = false;
    console.error(
      `Cannot export: the selected format ${format.value} is neither a runnable profile nor a custom command.`,
    );
  } else {
