...
function parseHostMessage (data: unknown): TikzEditorHostMessage|null {
  if (typeof data === 'string') {
    try {
      ...
      return parsed !== null && typeof parsed === 'object' ? parsed as TikzEditorHostMessage : null
    } catch {
      return null
    }
  }
...
function applyEditorSource (source: string, revision: number|undefined): void {
  ...
  if (source === active.source.source) return
  ...
  if (current !== active.source.source) return
...
function onMessage (event: MessageEvent): void {
  ...
  switch (message.event) {
    case 'init':
      ...
      break
    case 'loaded':
      if (message.revision !== session.value.revision) break
      loaded.value = true
      errorMessage.value = ''
      break
    case 'change':
    ...
    case 'save': {
      const source = typeof message.source === 'string'
        ? message.source
        : typeof message.xml === 'string' ? message.xml : null
      if (source !== null) applyEditorSource(source, message.revision)
      break
    }
    case 'message':
...
const statusText = computed(() => {
  ...
  if (!hostReady.value) return 'Loading tikz-editor…'
