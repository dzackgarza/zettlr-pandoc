...
const snippetsExplanation = trans(
  "Snippets let you define reusable pieces of text with variables.",
);
...
function updateAvailableSnippets(selectAfterUpdate?: string): void {
  ipcRenderer
    .invoke("assets-provider", { command: "list-snippets" })
    .then((data) => {
      availableSnippets.value = data;
      if (
        typeof selectAfterUpdate === "string" &&
        availableSnippets.value.includes(selectAfterUpdate)
      ) {
...
function addSnippet(newName?: string): void {
  ...
  ipcRenderer
    ...
    .then(() => {
      updateAvailableSnippets(newName);
    })
    .catch((err) => console.error(err));
}
...
function renameSnippet(): void {
  ...
  ipcRenderer
    ...
    .then(() => {
      updateAvailableSnippets(newVal);
    })
    .catch((err) => console.error(err));
}
