...
const langIntro = computed(() => {
  return trans(
    "Based on your operating system settings, we have selected %s as the application language. If you wish, you can select a different language here.",
    appLangOptions.value[originalLanguage],
  );
});
...
function changeAppLang() {
  ...
  } as OnboardingIPCSetAppLangMessage;
  ipcRenderer
    .invoke("onboarding", msg)
    .then(() => {
      loadData()
