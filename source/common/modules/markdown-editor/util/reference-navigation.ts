 ...
 */
export function captureDocumentLocation (view: EditorView, documentPath: string): DocumentLocation | null {
 ...
 */
function resolveCitationIntent (view: EditorView, citation: SyntaxNode, pos: number, documentPath: string, preferredKey?: string): ReferenceNavigationIntent|null {
  const references = view.state.field(workspaceReferencesField, false)
  if (references === null || references === undefined) {
 ...
 */
function resolveFootnoteIntent (view: EditorView, footnote: SyntaxNode, documentPath: string): ReferenceNavigationIntent|null {
 ...
 */
export function resolveReferenceNavigationIntent (view: EditorView, pos: number, preferredKey?: string): ReferenceNavigationIntent | null {
 ...
 */
export function followReferenceNavigationIntent (view: EditorView, intent: ReferenceNavigationIntent): boolean {
 ...
 * editor.
 */
function requestHistoryNavigation (view: EditorView, command: 'navigate-back'|'navigate-forward'): boolean {
  const { windowId, leafId } = view.state.facet(editorMetadataFacet)
