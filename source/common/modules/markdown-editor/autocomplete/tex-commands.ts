...
function argumentScaffold (label: string, argumentCount: number|undefined): string {
  if (argumentCount === undefined || argumentCount === 0) return `${label}$0`
...
async function userMathJaxCommands (): Promise<readonly UserMathJaxCommand[]> {
  if (userMathJaxCommandPromise === null) {
    userMathJaxCommandPromise = window.ipc.invoke('mathjax-macros')
      .then(macros => Object.entries(macros).map(([name, definition]) => ({
        label: `\\${name}`,
        definition
      })))
  }
...
function macroInfo (macro: UserMathJaxCommand): string {
  ...
  const signature = optionalDefault === undefined
...
function macroDetail (macro: UserMathJaxCommand): string {
  if (typeof macro.definition === 'string') return 'user macro'
...
function macroInvocationTemplate (macro: UserMathJaxCommand): string {
  if (typeof macro.definition === 'string') return `${macro.label}$0`
  const [, argumentCount, optionalDefault] = macro.definition
  const fields: string[] = []
  let next = 1
  if (optionalDefault !== undefined) {
...
function latexWorkshopOptions (): Completion[] {
  ...
  for (const [key, untypedEntry] of Object.entries(latexWorkshopCommands)) {
    ...
    options.push(withCompletionSource({
      label: `\\${name}`,
      type: 'keyword',
      detail: entry.detail,
...
function environmentTemplateSource (environment: LatexWorkshopEnvironment): string {
  ...
  const body = hasSeparateArgument
    ? '\n\t$0'
    : (environment.arg?.snippet ?? '\n\t$0')
  return `\\begin{${environment.name}}${argument}${body}\n\\end{${environment.name}}`
}

function latexEnvironmentOptions (): Completion[] {
  return (latexWorkshopEnvironments as LatexWorkshopEnvironment[]).map(environment => withCompletionSource({
...
async function commandOptions (inTikz: boolean, inTikzCd: boolean): Promise<Completion[]> {
  ...
  for (const label of mathJaxCompletionCatalogue().commands) {
    if (!standardLabels.has(label)) {
      const sourced = withCompletionSource({ label, type: 'keyword', detail: 'MathJax command', boost: 2 }, 'MathJax')
  ...
  for (const macro of await userMathJaxCommands()) {
    ...
    if (!options.some(option => option.label === macro.label && option.detail?.startsWith('user macro'))) {
  ...
  if (inTikz) {
    for (const label of TIKZ_CONTROL_WORDS) {
      if (!standardLabels.has(label)) {
        const sourced = withCompletionSource({ label, type: 'keyword', detail: 'TikZ command', boost: 8 }, 'TikZ')
    ...
    for (const macro of await tikzTemplateCommands()) {
      ...
      options.push(withCompletionSource({
        label: macro.label,
        type: 'keyword',
        detail: macro.argumentCount === undefined
          ? 'TikZ template macro'
        ...
        boost: 30,
        apply: macro.argumentCount === undefined
          ? macro.label
          : (view, completion, from, to) => { codeMirrorSnippet(invocation)(view, completion, from, to) }
      }, 'Macro'))
    }
  }
  if (inTikzCd) {
    for (const label of TIKZCD_CONTROL_WORDS) {
      const sourced = withCompletionSource({ label, type: 'keyword', detail: 'tikz-cd command', boost: 8 }, 'tikzcd')
  ...
  if (environment !== null) {
    ...
    return {
      ...
      options: [
        ...
        ...mathJaxCompletionCatalogue().environments
          .filter(label => !standardNames.has(label))
          .map(label => {
            const sourced = withCompletionSource({ label, type: 'type' as const, detail: 'MathJax environment' }, 'MathJax')
            return withDefaultCompletionInfo(sourced, 'MathJax', `\\begin{${label}}\n\t…\n\\end{${label}}`)
          })
      ]
    }
  }
