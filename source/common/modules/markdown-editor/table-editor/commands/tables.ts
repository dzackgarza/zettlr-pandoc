...
import { type ChangeSpec, EditorSelection, type TransactionSpec } from "@codemirror/state";
import type { EditorView } from "@codemirror/view";
import type { SyntaxNode } from "@lezer/common";
import {
  findColumnIndexByRange,
  getDelimiterLineCellOffsets,
  mapSelectionsWithTables,
} from "./util";
...
export function setAlignment(
  ...
  return (target) => {
    const changes = mapSelectionsWithTables<ChangeSpec>(target, (ctx) => {
      ...
      const delimChar =
        ctx.tableAST.tableType === "grid" ? "+" : delimLine.includes("|") ? "|" : "+";
      ...
      switch (alignTo) {
        case "left":
          return {
            from: node.from + from,
            to: node.from + to,
            insert: ":" + fillChar.repeat(Math.max(to - from - 1, 2)),
          };
        case "right":
          return {
            from: node.from + from,
        ...
        case "center":
          return {
            from: node.from + from,
            to: node.from + to,
            insert: ":" + fillChar.repeat(Math.max(to - from - 2, 1)) + ":",
          };
        default:
          return {
            from: node.from + from,
            to: node.from + to,
            insert: fillChar.repeat(Math.max(to - from, 3)),
          };
      }
    });
...
export function clearTable(target: EditorView): boolean {
  const changes = mapSelectionsWithTables<ChangeSpec[]>(target, (ctx) => {
    ...
    return ctx.offsets.outer.flatMap((row) =>
      row.map(([from, to]) => ({ from, to, insert: " ".repeat(to - from) })),
    );
    ...
    for (let i = 0; i < cellContents.length; i++) {
      ...
      for (let j = 0; j < cellContents[0].length; j++) {
        ...
        switch (ctx.tableAST.alignment[j]) {
          ...
          case "center":
            newContents[i].push(
              " ".repeat(Math.floor(pad / 2)) + cell + " ".repeat(Math.ceil(pad / 2)),
            );
            break;
          case "left":
          default:
