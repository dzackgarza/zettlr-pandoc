...
function insertPandocDiv(target: EditorView, attributes: string): void {
  ...
  const transaction = target.state.changeByRange((range) => {
    ...
    return {
      changes,
      range: EditorSelection.range(range.from + opening.length, range.to + opening.length),
    };
  });
 ...
 * @return  {boolean}                 Whether the command was applicable
 */
export function applyPandocDivOrSpan(
  target: EditorView,
  type: "div" | "span",
...
export function insertTabOrSpace(target: EditorView): boolean {
  ...
  if (
    ...
    nodeInSelection(
      ...
      tree,
      ["YAMLFrontmatter", "YAMLFrontmatterStart", "YAMLFrontmatterEnd"],
      -1,
    )
  ) {
  ...
  if (alwaysIndentLineOnTab || target.state.selection.ranges.some((r) => !r.empty)) {
    ...
    target.dispatch(target.state.replaceSelection(insert), {
      scrollIntoView: true,
      userEvent: "input",
    });
    result = true;
