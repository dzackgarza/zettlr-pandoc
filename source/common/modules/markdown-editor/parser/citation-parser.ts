...
export const citationParser: InlineParser = {
  ...
  parse: (ctx, next, pos) => {
    ...
    const validBefore =
      Number.isNaN(prevChar) ||
      [CHAR.BRACE_OPEN, CHAR.LF, CHAR.CR, CHAR.TAB, CHAR.SPACE].includes(prevChar);
    if (!validBefore) {
      return -1;
    }
    ...
    if (next === CHAR.BRACKET_OPEN) {
      ...
      for (; /* i is at the correct position */ i < ctxEndPos; i++) {
        ...
        if (
          citekeyStart < 0 &&
          i === citationPartStart &&
          [CHAR.SPACE, CHAR.LF, CHAR.CR, CHAR.TAB].includes(ch)
        ) {
        ...
        if (
          ch === CHAR.AT &&
          citekeyStart < 0 &&
          [CHAR.SPACE, CHAR.HYPHEN, CHAR.BRACKET_OPEN, CHAR.SEMICOLON].includes(prevCh)
        ) {
        ...
        if (citekeyStart > -1 && citekeyEnd < 0) {
          ...
          if (i === citekeyStart && ch === CHAR.CURLY_OPEN) {
            ...
            // TODO: I would like to avoid string conversion here.
        ...
        const startsImplicitLocator =
          (ch >= 48 && ch <= 57) || startsWithRomanNumeralLocator(ctx.slice(i, ctxEndPos));
        ...
        const explicitLabel =
          lclocIndex > 0 && allValidLocatorLabels.has(lcloc) ? lcloc : undefined;
        ...
        if (locatorStart > -1 && locatorEnd < 0) {
          ...
          if (locatorInBrackets && ch === CHAR.CURLY_CLOSE) {
            ...
            (ch < 48 || ch > 57) &&
            !ROMAN_NUMERAL_CODES.includes(ch) &&
            ch !== CHAR.HYPHEN
          ) {
      ...
      if (
        i < ctxEndPos - 1 &&
        ctx.char(i) === CHAR.SPACE &&
        ctx.char(i + 1) === CHAR.BRACKET_OPEN
      ) {
        ...
        if (explicitLabel !== undefined) {
          ...
          (ctx.char(i) >= 48 && ctx.char(i) <= 57) ||
          ROMAN_NUMERAL_CODES.includes(ctx.char(i))
        ) {
