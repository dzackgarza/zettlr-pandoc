...
import type { BlockParser, DelimiterType, InlineParser } from "@lezer/markdown";
  ...
  parse: (ctx, next, pos) => {
    ...
    if (opening === null) {
      ...
      return invalidOpening
        ? -1
        : ctx.addDelimiter(MathDelimiter, pos, pos + delimLength, true, false);
    }
    ...
    if (
      !isInlineDisplayMath &&
      (/\s/.test(ctx.slice(pos - 1, pos)) || /\d/.test(ctx.slice(pos + 1, pos + 2)))
    ) {
    ...
    return ctx.addElement(
      ctx.elt("InlineCode", delim.from, pos + delimLength, [
        openingMark,
        innerElements,
        closingMark,
      ]),
...
export const inlineBracketMathParser: InlineParser = {
  ...
  parse: (ctx, next, pos) => {
    ...
    const innerElements = ctx.elt(
      stexLang.parser.parse(ctx.slice(contentFrom, contentTo)),
      contentFrom,
    );
    ...
    return ctx.addElement(
      ctx.elt("InlineCode", pos, closeTo, [openingMark, innerElements, closingMark]),
    );
  },
};
...
export const blockMathParser: BlockParser = {
  ...
  parse: (ctx, line) => {
    ...
    ctx.addElement(
      ctx.elt("FencedCode", blockStart, closeMarkTo, [openingMark, codeText, closingMark]),
    );
    ctx.nextLine();
