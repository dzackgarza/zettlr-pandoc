...
export const pandocLinkParser: InlineParser = {
  ...
  parse: (ctx, next, pos) => {
    ...
    if (title?.indices?.groups) {
      ...
      const linkTitleIndices =
        title.indices.groups.double ?? title.indices.groups.single ?? title.indices.groups.parens;
      urlContents.push(
        ctx.elt("LinkTitle", pos + 2 + linkTitleIndices[0], pos + 2 + linkTitleIndices[1]),
      );
    }
    ...
    return ctx.addElement(
      ctx.elt(isLink ? "Link" : "Image", delim.from, pos + 3 + url.length, children),
    );
  },
};
