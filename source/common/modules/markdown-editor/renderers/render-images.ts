...
function normalizeSize(size?: string): string | undefined {
  if (
    size === undefined ||
    !/[\d\.]+(?:cm|mm|in|px|pt|pc|em|ex|ch|rem|vw|vh|vmin|vmax|%)/i.test(size)
  ) {
    return undefined;
  }
...
class ImageWidget extends WidgetType {
  ...
  eq(other: ImageWidget) {
    return (
      this.imageTitle === other.imageTitle &&
  ...
  toDOM(view: EditorView): HTMLElement {
    ...
    const maxHeight =
      normHeight !== undefined ? `min(${normHeight}, ${defaultHeight})` : defaultHeight;
