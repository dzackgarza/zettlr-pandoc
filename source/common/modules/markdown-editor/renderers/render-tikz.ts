...
class TikzWidget extends WidgetType {
  ...
  toDOM(view: EditorView): HTMLElement {
    ...
    requestRender({ source: this.source, kind: this.kind, docPath }).then(
      (result) => {
        populate(elem, result);
      },
    ...
    elem.addEventListener("click", () => {
      ...
      elem.ownerDocument.dispatchEvent(
        new ownerWindow.CustomEvent("zettlr-tikz-lightbox", { detail: { svgPath } }),
      );
    });
    return elem;
  }
