...
export function mainEditorKeybindings (customShortcutMap: CustomEditorShortcut[], config: Pick<EditorConfiguration, 'autocompleteWithEnter'|'autocompleteWithTab'>): KeyBinding[] {
  ...
  return [
    ...
    // Per-pane session-history Forward takes the Alt-Arrow (Ctrl-Arrow on
    // macOS) default, so cursorSyntaxRight moves down to Mod-Alt-Arrow. The
    // command returns false in views without editor metadata (window/leaf
    // IDs), so non-main editors fall through to the next binding.
    { key: sc('nav-history-forward'), run: navigateHistoryForward, preventDefault: true },
    { key: 'Mod-Alt-ArrowRight', run: cursorSyntaxRight },
    ...
    // See the Forward binding above: cursorSyntaxLeft moves to Mod-Alt-Arrow.
    { key: sc('nav-history-back'), run: navigateHistoryBack, preventDefault: true },
    { key: 'Mod-Alt-ArrowLeft', run: cursorSyntaxLeft },
    ...
    // Resolve the locale when the command runs, not when the keymap is built:
    // subview keymaps are constructed outside the renderer, where window.config
    // does not exist, and Intl.Segmenter rejects the resulting locale.
    { key: sc('tr-sentence-case'), run: view => toSentenceCase(String(window.config.get('appLang')))(view) },
    { key: sc('tr-title-case'), run: view => toTitleCase(String(window.config.get('appLang')))(view) },
    ...
    { key: sc('md-format-document'), run: requestFormatDocument },
