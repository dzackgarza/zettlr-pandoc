 ...
 * BEGIN HEADER
 *
 * Contains:        Scholarly document linter
 * CVM-Role:        Linter
 * Maintainer:      D. Zack Garza
 * License:         GNU GPL v3
 *
 * Description:     High-signal diagnostics specific to mathematical/scholarly
 *                  Markdown which cannot be repaired by formatting: configured
 *                  TeX macro validity, mathematical notation consistency, and
 *                  authorial residue. Reference/citation resolution remains in
 *                  reference-lint; Markdown/Pandoc syntax remains in Flowmark.
 *
 * END HEADER
 */
...
import type { TexResourceKind } from "source/app/util/tex-resource-resolver";
...
function collectDocumentRegions(markdown: string): {
  math: SourceRegion[];
  literalCode: Array<{ from: number; to: number }>;
  texIgnored: Array<{ from: number; to: number }>;
  prose: SourceRegion[];
} {
  const math: SourceRegion[] = [];
  const literalCode: Array<{ from: number; to: number }> = [];
  ...
  walkAST(markdownToAST(markdown), (node) => {
    if (node.type === "InlineCode" || node.type === "FencedCode") {
      ...
      if (parsedMath === null) {
        literalCode.push({ from: node.from, to: node.to });
  ...
  return { math, literalCode, texIgnored, prose };
}
...
function localMacroNames(
  ...
  // CodeMirror offsets are UTF-16 code-unit offsets, so preserve that indexing
  // while blanking ignored source ranges (Array.from would count code points).
...
function notationDiagnostics(regions: SourceRegion[]): Diagnostic[] {
  const diagnostics: Diagnostic[] = [];
  for (const [first, second] of NOTATION_VARIANTS) {
    ...
    if (firstOffsets.length === 0 || secondOffsets.length === 0) {
      continue;
    }
    if (firstOffsets.length === secondOffsets.length) {
      continue;
    }
...
function configuredMacroExpansionDiagnostics(
  ...
  for (const region of regions) {
    for (const [expansion, macros] of aliases) {
      // Several aliases for one expansion do not establish which spelling is
      // canonical, so the linter has no authorial decision to enforce.
      if (macros.length !== 1) {
        continue;
      }
...
function residueDiagnostics(regions: SourceRegion[]): Diagnostic[] {
  const diagnostics: Diagnostic[] = [];
  for (const region of regions) {
...
async function texResourceDiagnostics(
  ...
  // In the production editor this field is installed before the asynchronous
  // workspace/Project snapshot arrives. Do not transiently misdiagnose a
  // Project-root resource as missing while that authority is still unknown.
  if (
  ...
  const projectRoots = referenceContext?.projectRoots?.map((root) => root.rootPath) ?? [];
  const results = await window.ipc.invoke("tex-resource-probe", {
    sourcePath,
    projectRoots,
    graphicRoots,
...
export async function scholarlyLintSource(view: EditorView): Promise<Diagnostic[]> {
  ...
  const resourceDiagnostics = await texResourceDiagnostics(view, markdown, regions.texIgnored);

  return [
    ...unknownMacroDiagnostics(regions.math, knownCommands, localMacros),
    ...notationDiagnostics(regions.math),
    ...configuredMacroExpansionDiagnostics(regions.math, configuredMacros),
