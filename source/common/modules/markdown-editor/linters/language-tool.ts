...
import type {
  LanguageToolLinterRequest,
  LanguageToolLinterResponse,
} from "@providers/commands/language-tool";
...
function refreshUserDictionary(): void {
  ...
  ipcRenderer
    .invoke("dictionary-provider", { command: "get-user-dictionary" })
...
export const languageToolState = StateField.define<LanguageToolStateField>({
  create: (state) => {
    ...
    if (
      ...
      /^[a-z]{2,3}(-[A-Z]{2,})?/.test(frontmatter.lang)
    ) {
...
const ltLinter = linter(
  async (view) => {
    ...
    for (const match of ltSuggestions.matches) {
      ...
      if (!textNodes.some((node) => matchFrom >= node.from && matchTo <= node.to)) {
        continue;
      }
      ...
      if (issueType === "misspelling" && userDictionary.has(word)) {
        continue;
      }

      const source = `language-tool(${issueType})`;
      const severity =
        issueType === "style" ? "info" : issueType === "misspelling" ? "error" : "warning";
...
export const languageTool = [ltLinter, languageToolState, languagetoolTheme];
