...
import { ChangeSet, type Extension, StateEffect } from "@codemirror/state";
...
export type PullUpdateCallback = (
  filePath: string,
  version: number,
) => Promise<SerializedUpdate[] | false>;
export type PushUpdateCallback = (
  filePath: string,
  version: number,
  ...
  const plugin = ViewPlugin.fromClass(
    class {
      ...
      constructor(private readonly view: EditorView) {
        ...
        // updates available.
        this.pull().catch((err) => {
          console.error(`Pulling updates failed: ${String(err.message)}`, err);
        });
      }
      ...
      update(update: ViewUpdate): void {
        ...
        if (update.docChanged) {
          this.push().catch((err) => {
            console.error(`Pushing updates failed: ${String(err.message)}`, err);
          });
        }
      }
