  ...
  update(value, tr) {
    ...
    if (tr.docChanged) {
      const suggestions = value.suggestions.flatMap((suggestion) => {
        const mapped = mapSuggestionThroughChanges(suggestion, tr.changes, (from, to) =>
          tr.startState.doc.sliceString(from, to),
        );
...
function reviewStatusPanel(view: EditorView): Panel {
  ...
  const previous = makeButton(
    "cm-reviewNav previous",
    "‹ Prev",
    "Previous chunk (Shift-F8)",
    () => {
  ...
  const runPanelAction = (action: () => Promise<void>): void => {
    busy = true;
    void withControlsLocked(dom, [acceptAll, clear, commentSubmit], action).finally(() => {
      busy = false;
      render(view.state);
    });
  };
...
/**
 * The text a chunk's note carries. A chunk with no note carries none, and the
 * empty string is what "none" reads as everywhere this is used: in the input's
 * value, in the committed text the dirty indicator compares against, and in
 * the empty text that removes a note.
 */
function noteText(note: ReviewChunkCommentView | undefined): string {
  return note === undefined ? "" : note.comment;
}
 ...
 */
function renderChunkMeta(container: HTMLElement, descriptions: readonly string[]): void {
...
class ChunkControlsWidget extends WidgetType {
  ...
  eq(other: ChunkControlsWidget): boolean {
    return (
      other.suggestion.suggestionId === this.suggestion.suggestionId &&
      ...
      other.suggestion.anchors.every((anchor, index) => {
        ...
        return (
          current.from === anchor.from &&
          current.to === anchor.to &&
          other.doc.sliceString(anchor.from, anchor.to) ===
            this.doc.sliceString(current.from, current.to)
        );
      }) &&
  ...
  toDOM(view: EditorView): HTMLElement {
    ...
    const decide = (decision: "accept" | "reject"): void => {
      void withControlsLocked(buttons, controls, async () => {
        ...
        await requireReviewChunksConfig(view.state).onDecide(
    ...
    noteInput.title =
      "Annotate this change without deciding it; clearing the field removes the note";
    noteInput.value = noteText(this.note);
    ...
    const tryCommit = (): void => {
      ...
      const current = noteText(
        config.chunkComments.find((candidate) => candidate.chunkId === chunkId),
      );
      ...
      state.inflight = true;
      config
        .onChunkComment(chunkId, text)
        .then(() => {
          state.committed = text;
        })
        .catch((err) => {
          console.error("Chunk comment refused", err);
        })
        .finally(() => {
  ...
  updateDOM(dom: HTMLElement): boolean {
    ...
    if (input !== null && state !== undefined && document.activeElement !== input) {
      const value = noteText(this.note);
      input.value = value;
