...
function showLineEndings(view: EditorView): DecorationSet {
  ...
  for (const { from, to } of view.visibleRanges) {
    for (let pos = from; pos <= to; ) {
      ...
      if (
        EXCLUDED_NODES.includes(node.name) ||
        (node.parent && EXCLUDED_NODES.includes(node.parent.name))
      ) {
        continue;
      }
 ...
 */
const pilcrowPlugin = ViewPlugin.fromClass(
  class {
...
const modeSwitcher = EditorState.transactionExtender.of((transaction) => {
  ...
  if (highlight === true) {
    return {
      effects: extensionCompartment.reconfigure([hw(), htw(), pilcrowPlugin, pilcrowTheme]),
    };
