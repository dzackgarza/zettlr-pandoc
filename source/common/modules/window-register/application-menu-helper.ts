...
export default function showPopupMenu(
  ...
  // are gone.
  if (cleanup) {
    // NOTE: we need a flag because of submenus
  ...
  if (process.platform === "darwin") {
    ...
    ipcRenderer
      ...
      .then((clickedID: string | undefined) => {
        ...
        if (foundItem === undefined) {
          ...
          console.warn(
            `Could not trigger action for context menu item ${clickedID}: Neither action nor callback provided.`,
          );
        }
      })
      .catch((err) => {
    ...
    return () => {
      /* Noop-function, since no cleanup is required */
    };
    ...
    if (item.type !== "submenu" && item.type !== "separator" && item.enabled !== false) {
      ...
      menuItem.addEventListener("mousedown", (event) => {
        ...
        if (item.action !== undefined) {
          ...
          console.warn(
            `Registered click on menu item "${item.label}", but it had neither an action, nor an ID attached to it.`,
          );
        }
