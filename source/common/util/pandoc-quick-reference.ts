 ...
 */
export const PANDOC_CROSSREF_PREFIXES = PANDOC_CROSS_REFERENCE_EXAMPLES.map(
...
export const REFERENCEABLE_DIV_CLASSES: readonly string[] = THEOREM_FAMILY_METADATA.map(
  (metadata) => metadata.divClass,
);
...
export const PANDOC_REFERENCE_AUTHORING_TOPICS: readonly ReferenceAuthoringTopic[] = [
  ...
  },
  {
    ...
    detail:
      "Jumps to the definition of the clicked reference; Back/Forward restores your previous location.",
    syntax: "Mod-Click",
  },
  {
    kind: "rename",
    ...
    detail:
      "Editing a definition key offers renaming every use of it across the workspace atomically.",
    syntax: "Edit #key",
  },
  {
 ...
 */
export const THEOREM_DIV_EXAMPLES: readonly TheoremDivExample[] = THEOREM_FAMILY_METADATA.map(
  (metadata) => ({
