...
export function coordsForRay(
  ...
  const [x1, y1, x2, y2, x3, y3] = [
    ...
    Math.cos(rad1) * innerRadius,
    Math.sin(rad1) * innerRadius,
    Math.cos(rad2) * outerRadius,
    Math.sin(rad2) * outerRadius,
    Math.cos(rad3) * innerRadius,
    Math.sin(rad3) * innerRadius,
  ];
