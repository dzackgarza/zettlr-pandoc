import { coordsForRay, mat3mul, rotationMatrix, scaleMatrix, translationMatrix } from "./util/math";
import { MAX_SUPPORTED_SEGMENTS, type Vec4, WebGLEngine } from "./webgl-engine";
...
export class IrisIndicator {
  ...
  private drawFrame() {
    ...
    const matTemp = mat3mul(translationMatrix(originX, originY), rotationMatrix(moveByRadians));

    const mat = mat3mul(matTemp, scaleMatrix(1, 1));
