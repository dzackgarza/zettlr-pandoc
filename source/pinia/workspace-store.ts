...
import {
  ...
  pathDirname,
  resolvePath,
} from "source/common/util/renderer-path-polyfill";
import type { AnyDescriptor, OtherFileDescriptor } from "source/types/common/fsal";
...
export const useWorkspaceStore = defineStore("workspace", () => {
  ...
  retrieveInitialUpdate(openPaths.value, workspaceMap, descriptorMap)
    .catch((err) =>
      console.error("[Workspace Store] Could not retrieve initial set of loaded paths", err),
    )
    .finally(() => {
      ...
      ipcRenderer.on("fsal-event", (_, payload: FSALEventPayload) => {
        console.log(
          ...
          `[WorkspaceStore] Received event ${payload.event}:${payload.path ?? payload.descriptor.path}`,
        );
        if (payload.event === "unlink" || payload.event === "unlinkDir") {
          ...
          payload.event === "add" ||
          payload.event === "addDir"
        ) {
          const root = [...workspaceMap.value.keys()].find((p) =>
            payload.descriptor.path.startsWith(p),
          );
  ...
  watch(lastLeafActiveFile, async () => {
    ...
    const shouldShowAttachment = (filePath: string): boolean => {
      ...
      return (
        (showDotFiles || !isDotFile(filePath)) &&
