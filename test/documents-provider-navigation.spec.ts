...
describe('Documents-provider navigation join (review C6)', function () {
  ...
  it('open-file with targetRange/sourceLocation stamps history and echoes the range', async function () {
    ...
    const openedOrigin = await invoke('open-file', { windowId, leafId, path: HALPHEN, newTab: true })
    ...
    assert.deepEqual(activation.targetRange, targetRange, 'the ACTIVE_FILE context must echo the definition targetRange')

    const navState = await invoke('get-navigation-state', { windowId, leafId }) as { canGoBack: boolean, canGoForward: boolean }
    assert.deepEqual(navState, { canGoBack: true, canGoForward: false }, 'the jump must widen the pane history backwards')
  })
  ...
  it('navigate-back returns the widened entry; forward restores the stamped destination', async function () {
    ...
    assert.equal(forwardEvent.filePath, THEOREMS, 'Forward must return to the jump target document')
    assert.deepEqual(
