...
describe("Reference definition badges (issue #1 Phase 4)", function () {
  ...
  function createEditor(
    documentPath: string,
    capturedRequests?: ReferenceSearchRequest[],
  ): { view: EditorView; payload: EditorWorkspaceReferences } {
  ...
  function countBadgeFor(view: EditorView, key: string): HTMLElement {
    const badge = view.dom.querySelector<HTMLElement>(
      `.reference-count-badge[data-reference-key="${key}"]`,
    );
  ...
  it("renders one subtle label badge per definition, showing the authored key", function () {
    ...
    assert.strictEqual(
      keys.length,
      15,
      "the fixture must define one target per supported theorem family",
    );
  ...
  it("renders a separate always-visible count badge for every definition, including zero", function () {
    ...
    assert.strictEqual(
      badges.length,
      payload.snapshot.definitions.length,
      "every definition carries its own count badge, even at zero citing locations",
    );
  ...
  it("keeps the authored definition source intact next to its badges", function () {
    ...
    for (const badge of [...labelBadges(view), ...countBadges(view)]) {
      assert.ok(
        !/\b(Theorem|Table|Figure|Equation|Section|Listing)\s+\d/.test(badge.textContent ?? ""),
        "badges never display a computed reference number",
      );
    }
  });
  ...
  it("emits the Mod-P signal family keyed to the definition when the count badge is clicked", function () {
    ...
    assert.strictEqual(
      torelli.dataset.citingCount,
      "2",
      "the emitted intent belongs to the badge that shows the citing count",
    );
  });
});
