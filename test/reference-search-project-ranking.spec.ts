...
function isCurrent (definition: ReferenceDefinition): boolean {
  const status = computeProjectReferenceStatus(definition.documentPath, ACTIVE_DOCUMENT, PROJECT_ROOTS)
...
describe('Mod-P current-Project-first ranking (review A3)', function () {
  ...
  it('lists every current-Project definition before every other definition on the empty query', function () {
    ...
    assert.ok(currentCount >= 5, `the fixture must stay representative (got ${currentCount} current-Project definitions)`)
    assert.ok(
  ...
  it('same-file definitions count as current for a standalone active document', function () {
    ...
    const currentIdentities = identities(results.filter((_definition, index) => {
      const status = computeProjectReferenceStatus(results[index].documentPath, standalonePath, PROJECT_ROOTS)
      return status === 'same-file' || status === 'in-active-project'
    }))
    const standaloneDefinitions = workspace.filter(definition => definition.documentPath === standalonePath)
    assert.ok(standaloneDefinitions.length > 0, 'the fixture standalone document must define at least one target')
    assert.deepStrictEqual(
