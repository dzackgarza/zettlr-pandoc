...
app
  ...
  .then(async () => {
    ...
    if (readiness !== "function") {
      throw new Error(
        `reference-navigation-entry did not initialize (refNavProbeMount is ${readiness})`,
      );
    }
    ...
    await js(
      window,
      `window.refNavProbeMount(${JSON.stringify(documents)}, ${JSON.stringify(halphenPath)})`,
    );
    ...
    if (parityChipBefore !== null) {
      ...
      parity = {
        ...
        chipSuppressedAfterClick:
          (await js(window, 'window.refNavProbeChipTarget("tbl:coble-lattices")')) === null,
        rawTokenVisibleAfterClick: await js(
          window,
          'window.refNavProbeHasVisibleText("@tbl:coble-lattices")',
        ),
      };
    }
    ...
    if (torelliTarget !== null) {
      const clickPos = await js(
        window,
        `window.refNavProbePosAtCoords(${torelliTarget.x}, ${torelliTarget.y})`,
      );
      ...
      crossModClick = {
        ...
        resolvedIntent:
          clickPos === null
            ? null
            : await js(window, `window.refNavProbeResolveIntentAt(${clickPos})`),
        preJumpLocation,
    ...
    await js(
      window,
      `window.refNavProbeMount(${JSON.stringify(documents)}, ${JSON.stringify(scenePath)})`,
    );
    await nextFrame(window);
    ...
    let footnoteParity = {
      tokenAvailable: footnoteTarget !== null,
      tokenRange: null,
      selectionAfter: null,
      invocations: [],
    };
    ...
    let sameModClick = {
      targetAvailable: localTarget !== null,
    ...
    if (localTarget !== null) {
      const clickPos = await js(
        window,
        `window.refNavProbePosAtCoords(${localTarget.x}, ${localTarget.y})`,
      );
      const invocationStart = (await js(window, "window.refNavProbeInvocations()")).length;
    ...
    let footnoteModClick = {
      targetAvailable: footnoteModTarget !== null,
      resolvedIntent: null,
      invocations: [],
      selectionAfter: null,
    };
    if (footnoteModTarget !== null) {
      const clickPos = await js(
        window,
        `window.refNavProbePosAtCoords(${footnoteModTarget.x}, ${footnoteModTarget.y})`,
      );
      const invocationStart = (await js(window, "window.refNavProbeInvocations()")).length;
    ...
    const sameFile = {
      footnoteParity,
      modClickReference: sameModClick,
      modClickFootnote: footnoteModClick,
