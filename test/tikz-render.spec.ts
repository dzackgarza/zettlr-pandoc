...
describe("TikZ TeX typography metadata", function () {
  it("reads the documentclass point size and defaults ordinary standalone documents to 10pt", function () {
    ...
    assert.strictEqual(
      latexBaseFontSizePt("\\documentclass[tikz,border=2pt,11pt]{standalone}\n"),
      11,
    );
  ...
  it("indexes macros from the owned template and its local package/input graph", async function () {
    ...
    try {
      ...
      assert.ok(
        !commands.includes("\\includegraphics"),
        "system-package commands belong to the standard LaTeX catalogue, not local template scanning",
      );
      ...
      assert.notStrictEqual(
        after,
        before,
        "editing a transitive macro file changes the render-context fingerprint",
      );
      assert.strictEqual(
...
describe("TikZ render service (issue #14)", function () {
  ...
  it("reports missing pdflatex/pdf2svg as a typed result, never silence", async function () {
    ...
    try {
      ...
      assert.ok(
        !result.ok && result.kind === "missing-tools",
        `expected missing-tools, got ${JSON.stringify(result).slice(0, 200)}`,
      );
  ...
  it("distinguishes a toolchain probe that failed from a tool that is absent", async function () {
    ...
    try {
      ...
      assert.ok(
        !result.ok,
        "a render whose toolchain could not be checked did not produce a figure",
      );
      assert.strictEqual(
        result.kind,
      ...
      if (result.kind === "toolchain-probe-failed") {
        ...
        assert.strictEqual(
          result.code,
          "EACCES",
          "the errno that distinguishes this failure from absence is carried",
        );
      }
    } finally {
  ...
  it("renders a tikzcd snippet to namespaced inline SVG plus a lightbox file, or reports the toolchain", async function () {
    ...
    if (!toolchainPresent) {
      assert.ok(
        !result.ok && result.kind === "missing-tools",
        "without the toolchain the typed missing-tools result is required",
      );
      return;
    }
    ...
    assert.ok(
      result.ok,
      `expected a successful render, got ${JSON.stringify(result).slice(0, 400)}`,
    );
    if (result.ok) {
      ...
      assert.ok(
        result.html.includes(`xlink:href="#${idMatch?.[1] ?? ""}-`),
        "use references are namespaced with the same prefix",
      );
      assert.ok(existsSync(result.svgPath), "a lightbox-servable SVG file exists");
      assert.ok(
        readFileSync(result.svgPath, "utf8").includes("<svg"),
        "the lightbox file is an SVG document",
      );
      assert.ok(
        result.svgPath.startsWith(cacheDir),
        "the SVG file lives in the app-owned cache dir",
      );
    }
  });
  ...
  it("renders a fenced tikzcd body through the owned template rather than treating it as a standalone TeX document", async function () {
    ...
    assert.ok(
      result.ok,
      `fenced tikzcd must compile as a template-owned snippet: ${JSON.stringify(result).slice(0, 400)}`,
    );
    if (result.ok) {
      assert.match(result.svg, /<svg/);
      assert.strictEqual(
        result.texFontSizePt,
        latexBaseFontSizePt(readFileSync(TIKZ_TEMPLATE, "utf8")),
      );
    }
  });
  ...
  it("invalidates identical figure source when a transitive template macro definition changes", async function () {
    ...
    try {
      ...
      const request = {
        source,
        kind: "raw" as const,
        language: "tikz" as const,
        docPath: NO_DOC_PATH,
      };
      const beforeFiles = new Set(
        readdirSync(cacheDir).filter((name) => /^dzgtikz-.*\.svg$/.test(name)),
      );
      const first = await renderTikz(request, {
        tikzAssetDir: TIKZ_ASSET_DIR,
      ...
      assert.ok(
        first.ok,
        `initial macro-backed render must succeed: ${JSON.stringify(first).slice(0, 400)}`,
      );
      ...
      assert.notStrictEqual(
        secondContext,
        firstContext,
        "the package edit changes the app-side render context",
      );

      const second = await renderTikz(request, {
        tikzAssetDir: TIKZ_ASSET_DIR,
        templatePath: template,
        cacheDir,
        env: process.env,
      ...
      assert.ok(
        second.ok,
        `same source must recompile under the changed macro graph: ${JSON.stringify(second).slice(0, 400)}`,
      );
      if (first.ok && second.ok) {
        assert.notStrictEqual(
          second.svg,
          first.svg,
          "the changed macro definition reaches pdflatex output",
        );
        assert.notStrictEqual(
          second.svgPath,
          first.svgPath,
          "Viewer.js receives a changed artifact path for changed output",
        );
      }

      const generated = readdirSync(cacheDir).filter(
        (name) => /^dzgtikz-.*\.svg$/.test(name) && !beforeFiles.has(name),
      );
      assert.strictEqual(
        generated.length,
        2,
  ...
  it("force-render bypasses the filter cache and rewrites the generated SVG", async function () {
    ...
    const generated = readdirSync(cacheDir).filter(
      (name) => /^dzgtikz-.*\.svg$/.test(name) && !before.has(name),
    );
    assert.strictEqual(
      generated.length,
      1,
      "this uncached source produces exactly one new filter-cache SVG",
    );
    ...
    assert.strictEqual(
      statSync(cachedSvg).mtimeMs,
      initialMtime,
      "ordinary repeat render leaves the filter cache file untouched",
    );
  ...
  it("maps a figure-compile failure back to the tikz source line", async function () {
    ...
    if (!result.ok) {
      assert.strictEqual(
        result.kind,
        "compile-error",
        `expected compile-error, got ${JSON.stringify(result).slice(0, 400)}`,
      );
      if (result.kind === "compile-error") {
        assert.ok(result.errors.length > 0, "the bang-error diagnostic is surfaced");
        const error = result.errors[0];
        assert.ok(
          error.message.toLowerCase().includes("undefined control sequence"),
          `the LaTeX message is carried: ${error.message}`,
        );
        assert.ok(
          error.sourceLine.includes("\\thisMacroDoesNotExist"),
          `the verbatim source line is carried: ${error.sourceLine}`,
        );
        assert.strictEqual(
          error.line,
          2,
          "the line is mapped into the figure body, not the generated .tex",
        );
        assert.match(
          result.log,
          /This is pdfTeX|This is LuaHBTeX|This is XeTeX/u,
          "the actual compiler transcript is carried, not only the mapped marker",
        );
        assert.match(
          result.log,
          /Undefined control sequence/u,
          "the compiler transcript preserves the TeX error text",
        );
      }
    }
  });
  ...
  it("resolves the figure \\input against the document the request names", async function () {
    ...
    await writeFile(
      path.join(secondDir, "figbody.tikz"),
      `X \\arrow[r, "g${stamp}"] & Y \\arrow[r, "h${stamp}"] & Z`,
    );
    ...
    assert.ok(
      fromFirst.ok,
      `the first document's include resolves, got ${JSON.stringify(fromFirst).slice(0, 400)}`,
    );
    assert.ok(
      fromSecond.ok,
      `the second document's include resolves, got ${JSON.stringify(fromSecond).slice(0, 400)}`,
    );
    if (fromFirst.ok && fromSecond.ok) {
      ...
      assert.ok(
        firstPrefix !== undefined && secondPrefix !== undefined,
        "both renders carry content-hash namespaced ids",
      );
      assert.notStrictEqual(
        firstPrefix,
        secondPrefix,
  ...
  it("renders a standalone \\input with a .tikz extension", async function () {
    ...
    const stamp = randomBytes(4).toString("hex");
    await writeFile(
      path.join(workDir, "diagram.tikz"),
    ...
    try {
      ...
      assert.ok(
        res.ok,
        `standalone \\input{.tikz} renders, got ${JSON.stringify(res).slice(0, 400)}`,
      );
  ...
  it("fails loud and immediately when a referenced \\input file is missing", async function () {
    ...
    const result = await renderTikz(
      {
        source: "\\input{nonexistent-missing-file.tikz}",
        kind: "raw",
        language: "tikz",
        docPath: NO_DOC_PATH,
      },
    ...
    if (!result.ok) {
      assert.strictEqual(
        result.kind,
        "compile-error",
        `expected compile-error, got ${JSON.stringify(result)}`,
      );
      if (result.kind === "compile-error") {
        assert.ok(result.errors.length > 0, "surfaces the missing input file diagnostic");
        assert.ok(
          result.errors[0].message.includes("Input file not found"),
          `message cites missing file: ${result.errors[0].message}`,
        );
  ...
  it("resolves a standalone \\input against the central figures directory", async function () {
    ...
    await writeFile(
      path.join(subDir, "shared-diagram.tikz"),
      `\\begin{tikzpicture}\n\\draw (0,0) -- (2,2) node[midway] {${stamp}};\n\\end{tikzpicture}\n`,
    );
    try {
      const res = await renderTikz(
        {
          source: "\\input{tikz/shared-diagram.tikz}",
          kind: "raw",
          language: "tikz",
          docPath: NO_DOC_PATH,
        },
      ...
      assert.ok(
        res.ok,
        `central figures \\input renders, got ${JSON.stringify(res).slice(0, 400)}`,
      );
      assert.ok(res.svg.includes("<svg"), "result contains SVG markup");
    } finally {
  ...
  it("reports a render killed by a signal as its own outcome naming the signal, not as a pandoc diagnostic", async function () {
    ...
    const pending = renderTikz(
      {
        source: uncachedTikzcd("A \\arrow[r] & B"),
        kind: "raw",
        language: "tikzcd",
        docPath: NO_DOC_PATH,
      },
    ...
    assert.ok(
      signalled > 0,
      "the render must actually spawn a pandoc process for this proof to mean anything",
    );
    ...
    if (!result.ok) {
      assert.strictEqual(
        result.kind,
        "render-terminated",
        `a signal kill is its own outcome, got ${JSON.stringify(result).slice(0, 400)}`,
      );
      if (result.kind === "render-terminated") {
        assert.strictEqual(
          result.signal,
          "SIGTERM",
          "the signal that ended the render is carried, not discarded",
        );
      }
    }
  });
  ...
  it("uses an explicitly configured TikZ data directory", async function () {
    ...
    await writeFile(
      path.join(configuredDir, "filters/tikzcd.lua"),
  ...
  it("defaults to the live ~/.pandoc tree when it carries the TikZ assets", async function () {
    ...
    await writeFile(
      path.join(userPandocDir, "filters/tikzcd.lua"),
      "-- ZETTLR_TIKZ_RENDER_PROTOCOL=3\n-- live user filter\n",
    );
    await writeFile(path.join(userPandocDir, "filters/utilities.lua"), "-- live user utilities\n");
    await writeFile(
      path.join(userPandocDir, "templates/standalone-tikz.tex"),
      "% live user template\n",
    );
