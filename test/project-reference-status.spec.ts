...
describe('Project reference status computation (issue #1 Phase 7)', function () {
  describe('computeProjectReferenceStatus', function () {
    ...
    it('reports same-file for a definition in the active document, winning over membership', function () {
      ...
      assert.strictEqual(
        computeProjectReferenceStatus(THEOREMS, THEOREMS, bothRoots),
        'same-file'
      )
    })
    ...
    it('classifies targets from a standalone active document', function () {
      ...
      assert.strictEqual(computeProjectReferenceStatus(THEOREMS, STANDALONE, bothRoots), 'another-project')
      assert.strictEqual(computeProjectReferenceStatus(OTHER_PAPER, STANDALONE, bothRoots), 'another-project')
      assert.strictEqual(
        computeProjectReferenceStatus(path.join(FIXTURE_ROOT, 'Other_Notes.md'), STANDALONE, bothRoots),
        'standalone'
      )
      assert.strictEqual(computeProjectReferenceStatus(STANDALONE, STANDALONE, bothRoots), 'same-file')
    })
    ...
    it('activates the containing root even when the active document is itself omitted', function () {
      ...
      assert.strictEqual(computeProjectReferenceStatus(THEOREMS, HALPHEN, roots), 'in-active-project')
      assert.strictEqual(computeProjectReferenceStatus(COBLE, HALPHEN, roots), 'omitted-from-active-project')
      assert.strictEqual(computeProjectReferenceStatus(OTHER_PAPER, HALPHEN, roots), 'another-project')
    })
  ...
  describe('annotateCompletionEntries', function () {
    it('computes the status of every real fixture definition (included active document)', function () {
      ...
      assert.ok(raw.length >= 10, 'the fixture workspace must provide a representative definition set')
  ...
  describe('appendToastMessage', function () {
    it('names both appended files in one toast for a source+target plan', function () {
      ...
      assert.ok(message.includes('Halphen_Surfaces.md'), `the toast must name the appended source: ${JSON.stringify(message)}`)
      assert.ok(message.includes('Coble_Lattice_Table.md'), `the toast must name the appended target: ${JSON.stringify(message)}`)
    })
    ...
    it('names the single appended file for a target-only plan', function () {
      ...
      assert.ok(message.includes('Coble_Lattice_Table.md'), `the toast must name the appended file: ${JSON.stringify(message)}`)
    })
  })

  describe('completionAffordanceFor', function () {
    const plan: AppendAndContinuePlan = { rootPath: PROJECT_A, appendFiles: [ 'Coble_Lattice_Table.md' ] }
    ...
    it('disables insertion for another-project entries', function () {
      assert.deepStrictEqual(
        completionAffordanceFor('another-project'),
    ...
    it('carries the append action for omitted entries with a plan', function () {
      assert.deepStrictEqual(
        completionAffordanceFor('omitted-from-active-project', plan),
    ...
    it('inserts plainly for omitted entries carrying no mechanical append plan', function () {
      assert.deepStrictEqual(
        completionAffordanceFor('omitted-from-active-project'),
        { kind: 'insert' }
      )
    })
    ...
    it('warns about the export unit for standalone entries', function () {
      assert.deepStrictEqual(
        completionAffordanceFor('standalone'),
