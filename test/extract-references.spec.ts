...
function tokenRange(content: string, token: string): SourceRange {
  ...
  assert.strictEqual(
    content.indexOf(token, from + 1),
    -1,
    `fixture token must be unique: ${token}`,
  );
...
const THEOREM_FIXTURE_DEFINITIONS: Array<{
  key: string;
...
describe("extractReferences()", function () {
  ...
  it("extracts a theorem-div definition for every supported prefix with exact ranges", function () {
    ...
    assert.deepStrictEqual(
      ...
      THEOREM_FIXTURE_DEFINITIONS.map((entry) =>
        definition(
          fixture,
          entry.key,
          "theorem-div",
          [theoremDivClass(entry.family)],
          entry.title,
  ...
  it("excludes proof divs, empty keys, and unsupported families from definitions", function () {
    const fixture = loadFixture(path.join("ProjectA", "Theorems.md"));
    const keys = extractReferences(fixture.documentPath, fixture.content).definitions.map(
      (def) => def.key,
    );
  ...
  it("extracts bare and bracketed occurrences with exact ranges and cluster text", function () {
    ...
    assert.strictEqual(
      fixture.content.includes(cluster),
      true,
      "fixture must contain the authored cluster",
    );
  ...
  it("extracts bracketed reference clusters but never bibliography citations", function () {
    ...
    assert.strictEqual(
      fixture.content.includes("[@Ols04, Lem. 7.1]"),
      true,
      "fixture must contain the citation",
    );
    ...
    assert.deepStrictEqual(
      occurrences.filter((occ) => occ.key === "Ols04"),
      [],
    );
    assert.deepStrictEqual(
      occurrences.filter((occ) => occ.clusterRaw.includes("@Ols04")),
      [],
    );
  });
  ...
  it("extracts duplicate definitions from every defining file", function () {
    ...
    const inTheorems = extractReferences(
      theorems.documentPath,
      theorems.content,
    ).definitions.filter((def) => def.key === "thm:torelli");
    const inOtherPaper = extractReferences(
      otherPaper.documentPath,
      otherPaper.content,
    ).definitions.filter((def) => def.key === "thm:torelli");

    assert.deepStrictEqual(
      inTheorems.map((def) => def.documentPath),
      [theorems.documentPath],
    );
  ...
  it("preserves escaped quotes inside authored title attributes (review C8)", function () {
    ...
    assert.strictEqual(
      ...
      1,
      "the escaped-quote div must still define its target",
    );
  ...
  it("never fabricates definitions from unclosed attribute-block near-misses (review C8)", function () {
    ...
    assert.deepStrictEqual(
      headingSnapshot.definitions,
      [],
      "an unclosed heading attribute block defines nothing",
    );
    assert.strictEqual(
      headingSnapshot.occurrences.length,
      1,
      "the citing occurrence itself must still extract",
    );
    ...
    assert.deepStrictEqual(
      divSnapshot.definitions,
      [],
      "an unclosed div attribute block defines nothing",
    );
  });
