...
const PANDOC_OUTPUT = "pandoc 3.9.0.2\nFeatures: +server +lua\n";
...
describe("pandoc-crossref preflight compatibility (issue #1 Phase 7)", function () {
  it("registers pandoc-crossref as a hard required command", function () {
    const entry = REQUIRED_COMMANDS.find(
      (requirement) => requirement.command === "pandoc-crossref",
    );
  ...
  it("parses matching version outputs as compatible, naming both versions", function () {
    assert.deepStrictEqual(assessCrossrefCompatibility(CROSSREF_OUTPUT_MATCHING, PANDOC_OUTPUT), {
      status: "compatible",
  ...
  it("detects a version mismatch and names both versions", function () {
    assert.deepStrictEqual(assessCrossrefCompatibility(CROSSREF_OUTPUT_MISMATCHED, PANDOC_OUTPUT), {
      status: "incompatible",
  ...
  it("produces a preflight failure naming both versions on mismatch", async function () {
    ...
    assert.ok(
      failure.includes("3.6.4"),
      `the failure must name the version pandoc-crossref was built with: ${JSON.stringify(failure)}`,
    );
    assert.ok(
      failure.includes("3.9.0.2"),
      `the failure must name the installed pandoc version: ${JSON.stringify(failure)}`,
    );
  });
  ...
  it("executes the real tools and reports the versions they print", async function () {
    ...
    const pandocVersion = pandocOut
      .split("\n")[0]
      .replace(/^pandoc(?:\.exe)?\s+/, "")
      .trim();
    assert.ok(
      /^\d+[.\d]*$/.test(pandocVersion),
      `pandoc must print a parseable version: ${JSON.stringify(pandocOut.split("\n")[0])}`,
    );
  ...
  it("preflight() itself fails the boot on a compatibility mismatch (review C1)", async function () {
    ...
    assert.ok(
      shown[0].message.includes("3.6.4"),
      `the boot failure must name the version pandoc-crossref was built with: ${JSON.stringify(shown[0].message)}`,
    );
    assert.ok(
      shown[0].message.includes("3.9.0.2"),
      `the boot failure must name the installed pandoc version: ${JSON.stringify(shown[0].message)}`,
    );
