...
const findObjectTester: Array<{ input: [any, string, any, string]; expected: any }> = [
  ...
  {
    input: [findObjectTree, "path", "/Users/hendrik/Documents/My Texts/My Texts.pdf", "children"],
    expected: undefined,
  },
  // This should work as the traversal occurs on attachments property
  {
    input: [findObjectTree, "hash", -1512390270, "attachments"],
    expected: findObjectTree.attachments[0],
  },
];
...
describe("Utility#findObject()", function () {
  for (let test of findObjectTester) {
    ...
    it(`should return ${expression}`, function () {
      ...
      assert.strictEqual(
        findObject(test.input[0], test.input[1], test.input[2], test.input[3]),
        test.expected,
      );
    });
  }
