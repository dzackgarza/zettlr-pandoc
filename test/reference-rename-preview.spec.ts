...
interface ProbeResult {
  ...
  cancelScene: { cancelClicked: boolean, events: { applyCount: number, closeCount: number } }|null
...
describe('Rename preview dialog', function () {
  ...
  before(async function () {
    ...
    assert.ok(jsonLine !== undefined, 'the rename-preview probe must return its observed result object')
  ...
  it('lists the exact affected files and counts from the fixture', function () {
    ...
    const expected = AFFECTED_FILES.map(relative => {
      ...
      const tokens = (source.split('#thm:torelli').length - 1) + (source.split('@thm:torelli').length - 1)
    ...
    for (const expectation of expected) {
      const row = rows.find(candidate => candidate.documentPath !== null && candidate.documentPath.endsWith(expectation.suffix))
      assert.ok(row !== undefined, `the preview must list ${expectation.suffix}`)
      assert.equal(row.editCount, expectation.editCount, `${expectation.suffix} must show its exact edit count`)
      assert.equal(row.snippets.length, expectation.editCount, `${expectation.suffix} must show one snippet per affected range`)
      for (const snippet of row.snippets) {
        assert.ok(snippet.includes('thm:torelli'), `every snippet must show the authored key in context, got ${JSON.stringify(snippet)}`)
      }
    }
  })
  ...
  it('Cancel commits nothing: no apply intent is ever emitted', function () {
    ...
    assert.equal(result.cancelScene.cancelClicked, true, 'the probe must find and click [data-cancel]')
    assert.deepEqual(
  ...
  it('Apply proceeds: exactly one apply intent for the host to commit', function () {
    ...
    assert.equal(result.applyScene.applyClicked, true, 'the probe must find and click [data-apply]')
    assert.equal(
