...
describe("Editor review-chunk view", function () {
        ...
        extensions: [
          reviewChunksExtension({
            ...
            onChunkComment: async (chunkId, text) => {
              calls.chunkNotes.push({ chunkId, text });
            },
          }),
  ...
  it("renders one decision control set per chunk and emits the addressed ids", function () {
    ...
    const suggestions = [
      ...
      replacementSuggestion(
        proposed.slice(proposed.indexOf("middle unchanged")),
        "baseline",
        "proposed",
        "second-change",
      ),
    ...
    const accepts = view.dom.querySelectorAll<HTMLButtonElement>(
      "button.cm-review-diff-control.accept",
    );
    const rejects = view.dom.querySelectorAll<HTMLButtonElement>(
  ...
  it("autosaves a trimmed chunk note after a typing pause, without blur", async function () {
    ...
    assert.equal(
      dot.classList.contains("unsaved"),
      true,
      "divergence flips the indicator at the keystroke",
    );
    ...
    assert.equal(
      dot.classList.contains("unsaved"),
      false,
      "the acknowledgment flips it back to saved",
    );
  ...
  it("keeps focus and unsent keystrokes across the commit echo redraw", async function () {
    ...
    const configFor = (
      chunkComments: ChunkNoteView[],
    ): Parameters<typeof reviewChunksExtension>[0] => ({
      reviewId: "review-test",
  ...
  it("shows each claim description and maps it across a working-text tweak", function () {
    ...
    const { view } = createReviewView(proposed, {
      suggestions: [
        ...
        {
          ...replacementSuggestion(
            proposed.slice(secondOffset),
            "baseline",
            "proposed",
            "closing",
            "Fix the closing claim",
          ),
  ...
  it("never renders controls over a region the user typed (#65)", function () {
    ...
    assert.equal(
      view.state.doc.sliceString(chunks[0].anchors[0].from, chunks[0].anchors[0].to),
      "proposed",
    );
  ...
  it("renders a small replacement as inline track changes with one controls strip", function () {
    ...
    assert.equal(
      ...
      null,
      "the delta renders in the document flow, not a block above it",
    );
  ...
  it("uses half-open chunk overlap while preserving deletion-point suppression", function () {
    ...
    assert.equal(rangeInPreviewSuppression(adjacent.state, unchanged.from, unchanged.to), false);
    ...
    assert.equal(rangeInPreviewSuppression(deletion.state, surviving.from, surviving.to), true);
    ...
    assert.equal(
      rangeInPreviewSuppression(deletionAtEnd.state, beforeDeletion.from, deletionChunks[0].seam),
      false,
    );
  });
  ...
  it("rebuilds preview renderers when a review arrives or clears without an edit", function () {
    ...
    assert.equal(
      rawMarkers(),
      false,
      "the link renders with hidden markers before a review exists",
    );

    view.dispatch({
      effects: reviewCompartment.reconfigure(
        reviewChunksExtension({
    ...
    assert.equal(rawMarkers(), false, "clearing the review without an edit re-renders the link");
  });
  ...
  it("decides through the configuration on screen, not the one its widget was built with", async function () {
    ...
    await new Promise((resolve) => setTimeout(resolve, 0));
    assert.deepEqual(current, [{ chunkId: chunks[0].suggestionId, decision: "accept" }]);
    ...
    const view = new EditorView({
      ...
      state: EditorState.create({
        ...
        extensions: [
          compartment.of(
            reviewChunksExtension(
              configFor([replacementSuggestion("proposal\n", "baseline", "proposal")]),
            ),
          ),
        ],
      }),
