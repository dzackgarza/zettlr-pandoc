...
describe("Searchable Pandoc quick help (review A2)", function () {
  it("narrows the cross-reference examples to the queried object", function () {
    const matches = filterHelpEntries(PANDOC_CROSS_REFERENCE_EXAMPLES, "tbl", (example) => [
      example.kind,
      example.label,
      example.reference,
    ]);
  ...
  it("narrows the theorem-div examples across class names and authored syntax", function () {
    const byClass = filterHelpEntries(THEOREM_DIV_EXAMPLES, "lemma", (example) => [
      example.divClass,
      example.label,
      example.reference,
    ]);
    assert.deepStrictEqual(
      byClass.map((example) => example.prefix),
      ["lem"],
    );

    const bySyntax = filterHelpEntries(THEOREM_DIV_EXAMPLES, "#conj:", (example) => [
      example.divClass,
      example.label,
      example.reference,
    ]);
    assert.deepStrictEqual(
      bySyntax.map((example) => example.prefix),
      ["conj"],
    );
  });

  it("matches case-insensitively across titles and details", function () {
    const matches = filterHelpEntries(PANDOC_REFERENCE_AUTHORING_TOPICS, "MOD-P", (topic) => [
      topic.title,
      topic.detail,
      topic.syntax,
    ]);
    assert.deepStrictEqual(
      matches.map((topic) => topic.kind),
      ["definition-search"],
    );
  });
  ...
  it("drops entries whose parts never contain the query", function () {
    assert.deepStrictEqual(
      filterHelpEntries(PANDOC_CITATION_EXAMPLES, "no-such-substring", (example) => [
        example.syntax,
      ]),
      [],
    );
  ...
  it("the help component consumes the shared filter behind a filter input", function () {
    ...
    assert.equal(
      source.includes("filterHelpEntries"),
      true,
      "the help must narrow through the shared filter",
    );
    assert.equal(
      source.includes("data-help-filter"),
      true,
      "the help must expose its text-filter input",
    );
  });
});
