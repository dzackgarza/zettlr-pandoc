...
describe("Create-reference-label dialog and key-edit prompt", function () {
  ...
  before(async function () {
    ...
    await execFileAsync(
      "node",
      [path.join(root, "test/reference-create-label-build.cjs"), outputDirectory],
    ...
    assert.ok(
      jsonLine !== undefined,
      "the create-label probe must return its observed result object",
    );
  ...
  it("leaving an edited definition id emits exactly one workspace-rename prompt intent", function () {
    ...
    assert.equal(
      intent.newKey,
      "thm:torelli-v2",
      "the intent must carry the authored replacement key",
    );
    assert.deepEqual(
      intent.range,
