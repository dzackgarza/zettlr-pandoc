...
function definedKeys (snapshots: DocumentReferenceSnapshot[]): Set<string> {
  return new Set(snapshots.flatMap(snapshot => snapshot.definitions.map(definition => definition.key)))
}
...
describe('Reference diagnostics (issue #1 Phase 4)', function () {
  ...
  function createEditor (doc: string, snapshot: DocumentReferenceSnapshot, workspace: DocumentReferenceSnapshot[]): EditorView {
  ...
  function createFixtureEditor (documentPath: string): { view: EditorView, doc: string, workspace: DocumentReferenceSnapshot[] } {
  ...
  function familyKeyTokens (message: string): string[] {
    return message.match(/\b(?:fig|tbl|eq|sec|lst|thm|lem|prop|cor|def|rmk|ex|conj|clm|obs|qst|prob|ass|warn|exr):[A-Za-z0-9:_-]+/g) ?? []
  }
  ...
  it('reports a duplicate key as an error listing ALL definition sites', async function () {
    ...
    const duplicate = diagnostics.find(diagnostic => diagnostic.severity === 'error' && diagnostic.from === from)
    assert.ok(duplicate !== undefined, 'the duplicated definition must receive an error at its authored id range')
    assert.strictEqual(duplicate.to, from + idToken.length)
    assert.ok(duplicate.message.includes(THEOREMS_PATH), 'the error must name this definition site')
    assert.ok(duplicate.message.includes(OTHER_PAPER_PATH), 'the error must name every other definition site, never a silently selected subset')

    // Unique definitions in the same document stay clean.
    for (const diagnostic of diagnostics) {
      assert.ok(!diagnostic.message.includes('lem:kodaira:embedding'), 'uniquely defined keys must not be flagged')
    }
  ...
  it('reports missing references as warnings at the authored occurrence without fabricating targets', async function () {
    ...
    const missingEmbedding = diagnostics.find(diagnostic => diagnostic.message.includes('lem:embedding'))
    ...
    const missingFigure = diagnostics.find(diagnostic => diagnostic.message.includes('fig:nonexistent-diagram'))
    assert.ok(missingFigure !== undefined, 'the undefined key fig:nonexistent-diagram must be reported')
    ...
    for (const [ diagnostic, missingKey ] of [ [ missingEmbedding, 'lem:embedding' ], [ missingFigure, 'fig:nonexistent-diagram' ] ] as Array<[Diagnostic, string]>) {
    ...
    for (const key of [ 'tbl:coble-lattices', 'eq:intersection-form', 'sec:moduli' ]) {
      ...
      assert.ok(!diagnostics.some(diagnostic => diagnostic.from === occurrenceFrom), `resolved key ${key} must not be flagged`)
    }
    assertNoActions(diagnostics)
  })
  ...
  it('reports a class/prefix mismatch naming BOTH conflicting values and a valid example', async function () {
    ...
    assert.ok(mismatch !== undefined, 'the conflicting definition must receive a diagnostic at its authored id range')
    ...
    assert.ok(mismatch.message.includes('thm:foo'), 'the error must name the authored conflicting id')
    assert.ok(mismatch.message.includes('#lem:foo'), 'the error must show a valid example per the THEOREM_DIV_PREFIXES mapping')
    assertNoActions(diagnostics)
  })
  ...
  it('reports an id on a proof div as an info diagnostic: proofs are unreferenceable', async function () {
    ...
    assert.ok(proof !== undefined, 'the proof-div id must receive a diagnostic at its authored token')
  ...
  it('reports a mixed bibliography/reference cluster as an advisory spanning the authored cluster', async function () {
    ...
    const advisory = diagnostics.find(diagnostic => diagnostic.from === clusterFrom && diagnostic.to === clusterTo)
    assert.ok(advisory !== undefined, 'the mixed cluster must receive an advisory spanning the whole authored cluster')
  ...
  it('reports nothing for a document whose definitions and occurrences all resolve cleanly', async function () {
    ...
    assert.deepStrictEqual(diagnostics, [], 'clean documents must produce zero reference diagnostics')
  })
})
