...
import {
  citekeyUpdate,
  citekeyUpdateField,
} from "source/common/modules/markdown-editor/autocomplete/citations";
...
describe("Reference diagnostics (issue #1 Phase 4)", function () {
  ...
  function createEditor(
    ...
    const state = EditorState.create({
      doc,
      selection: { anchor: 0 },
      extensions: [markdownParser(), configField, workspaceReferencesField, citekeyUpdateField],
    });
    ...
    view.dispatch({
      effects: [
        workspaceReferencesUpdate.of(payload),
        ...(citationKeys === undefined
          ? []
          : [citekeyUpdate.of(citationKeys.map((citekey) => ({ citekey, displayText: citekey })))]),
      ],
    });
    views.push(view);
    return view;
  ...
  it("distinguishes a cross-reference family mismatch from an ordinary missing key", async function () {
    const targetDoc = "![Main diagram](main.pdf){#fig:main}\n";
    const sourceDoc = "Equation @eq:main is used below.\n";
    const target = extractReferences("figures.md", targetDoc);
    const source = extractReferences("source.md", sourceDoc);
    const view = createEditor(sourceDoc, source, [target, source]);
    const diagnostics = await referenceLintSource(view);
  ...
  it("reports theorem divs whose class semantics are ambiguous or absent", async function () {
    const doc = [
      "::: {.theorem .lemma #thm:ambiguous}",
      "Ambiguous theorem family.",
      ":::",
      "",
      ...
      "",
      "::: {.note #thm:untyped}",
      "Theorem-family id without theorem class.",
      ":::",
      "",
    ].join("\n");
    const snapshot = extractReferences("theorem-structure.md", doc);
    const view = createEditor(doc, snapshot, [snapshot]);
    const diagnostics = await referenceLintSource(view);
  ...
  it("reports bibliography keys missing from the configured citation snapshot", async function () {
    const doc = "Compare [@Known2020; @Missing2021].\n";
    const snapshot = extractReferences("citations.md", doc);
    const view = createEditor(doc, snapshot, [snapshot], ["Known2020"]);
    const diagnostics = await referenceLintSource(view);

    const missingFrom = doc.indexOf("@Missing2021");
    const missing = diagnostics.find((diagnostic) => diagnostic.from === missingFrom);
    assert.ok(
      missing !== undefined,
      "the missing bibliography key must be diagnosed at its authored token",
    );
    assert.equal(missing.severity, "error");
    assert.equal(missing.to, missingFrom + "@Missing2021".length);
    assert.match(missing.message, /bibliograph/i);
    assert.equal(
      diagnostics.some((diagnostic) => diagnostic.from === doc.indexOf("@Known2020")),
      false,
      "a key present in the configured bibliography must stay clean",
    );
  });

  it("does not claim citations are missing before the bibliography snapshot arrives", async function () {
    const doc = "Compare [@NotLoadedYet].\n";
    const snapshot = extractReferences("citations-loading.md", doc);
    const view = createEditor(doc, snapshot, [snapshot]);
    const diagnostics = await referenceLintSource(view);
    assert.equal(
      diagnostics.some((diagnostic) => /bibliograph/i.test(diagnostic.message)),
      false,
      "startup must not turn an unavailable citation snapshot into false missing-key errors",
    );
  });
