...
describe("Modules#PersistentDataContainer", function () {
  ...
  it("should properly handle initialization", async function () {
    ...
    await assert.rejects(
      ...
      "The container did not complain about calling get()",
    );
    ...
    await assert.doesNotReject(
      async () => await container.get(),
      "The container complained despite being initialized",
    );
    container.shutdown();
  });
  ...
  it("should return the same data as was provided", async function () {
    ...
    assert.deepStrictEqual(
      retData,
      testData,
      "The provided and returned data was not the same (JSON)",
    );
    container1.shutdown();
    ...
    assert.deepStrictEqual(
      retData,
      testData,
      "The provided and returned data was not the same (YAML)",
    );
    container2.shutdown();
  });
  ...
  it("should not accept null or undefined as data", async function () {
    ...
    await assert.rejects(
      async () => await container.init(undefined),
      "The container did not reject undefined init data",
    );
    await assert.rejects(
      async () => await container.init(null),
      "The container did not reject null init data",
    );
