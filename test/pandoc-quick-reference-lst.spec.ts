...
describe("lst crossref family (issue #1 Phase 8)", function () {
  ...
  it("exposes a listing example pairing the lst label with the reference that cites it", function () {
    ...
    assert.ok(
      listing !== undefined,
      "the quick reference must document the listing family alongside fig/tbl/eq/sec",
    );
    assert.equal(listing.kind, "listing");
    assert.match(
      listing.label,
      /\{#lst:key\b/,
      "the listing example must author an explicit #lst:key attribute",
    );
  ...
  it("renders @lst references through the textual crossref branch in field-less states", function () {
    ...
    assert.ok(
      rendered !== null,
      "the cluster must render a citation widget in the field-less state",
    );
    assert.equal(
  ...
  it("renders a bracketed empty-slug cluster through the textual branch even WITH the field (review B5/B9)", function () {
    ...
    assert.ok(
      rendered !== null,
      "the empty-slug cluster must render a citation widget despite the field being present",
    );
    assert.equal(
      rendered.textContent,
      "#",
      'the suppress-author empty-slug form renders its textual "#<label>" shape',
    );
    assert.equal(
      rendered.classList.contains("error"),
      false,
      "the cluster must never surface as a failed bibliography citation",
    );
  });
  ...
  it("END-TO-END CANARY (already green): a resolved @lst occurrence renders as a chip", function () {
    ...
    assert.equal(
      chips.length,
      1,
      "the single resolved lst occurrence must render exactly one chip",
    );
    ...
    assert.ok(
      !/\d/.test(chips[0].textContent ?? ""),
      "the chip must never display a computed number",
    );
  });
});
