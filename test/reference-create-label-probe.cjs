...
app
  ...
  .then(async () => {
    ...
    const readiness = await window.webContents.executeJavaScript(
      "typeof window.createLabelProbeMount",
    );
    if (readiness !== "function") {
      throw new Error(
        `reference-create-label-entry did not initialize (createLabelProbeMount is ${readiness})`,
      );
    }
    ...
    if (mountReport.componentAvailable === true) {
      ...
      await nextFrame(window);
      createIntents = await window.webContents.executeJavaScript(
        "window.createLabelProbeCreateIntents()",
      );
