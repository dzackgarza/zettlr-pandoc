...
function servedSnapshot(
  ...
  assert.strictEqual(
    matches.length,
    1,
    `the merged view must serve exactly one snapshot for ${documentPath}`,
  );
  return matches[0];
}
...
describe("ReferenceIndex overlay authority", function () {
  ...
  it("always serves the LAST reported live buffer — the authority feed admits no counter to race (issue #53)", function () {
    ...
    const windowA = liveSnapshot(
      HALPHEN,
      (content) => content + "\nWindow A draft citing @fig:root-diagram.\n",
    );
    const windowB = liveSnapshot(
      HALPHEN,
  ...
  it("keeps the overlay authoritative across saved snapshots — only the authority ends the live side", function () {
    ...
    const edit = (content: string): string =>
  ...
  it("reverts to the saved snapshot when the live buffer is dropped, and reports whether an overlay existed", function () {
    ...
    assert.strictEqual(
      index.dropLiveBuffer(HALPHEN),
      true,
      "dropping an existing overlay must report the state change",
    );
    assert.deepStrictEqual(servedSnapshot(index.getSnapshot(), HALPHEN), saved);
    assert.strictEqual(
      index.dropLiveBuffer(HALPHEN),
      false,
      "a second drop has no overlay left to remove",
    );
  });
  ...
  it("follows a file move (unlink + add) to the new path without duplicating the definition (review C8)", function () {
    ...
    assert.strictEqual(
      resolution.status,
      "resolved",
      "the moved definition must resolve, not duplicate or go missing",
    );
    if (resolution.status === "resolved") {
      assert.strictEqual(
        resolution.definition.documentPath,
        movedPath,
        "the resolution must point at the NEW path",
      );
    }
  });
  ...
  it("resolves a saved occurrence through a definition that exists only in a live buffer", function () {
    ...
    assert.strictEqual(
      servedResolution(index.getSnapshot(), "fig:nonexistent-diagram").status,
      "missing",
    );
  ...
  it("reports a duplicate introduced by a live overlay with every definition retained", function () {
    ...
    assert.strictEqual(
      servedResolution(index.getSnapshot(), "rmk:characteristic-two").status,
      "resolved",
    );
    ...
    assert.deepStrictEqual(
      ...
      ["rmk:characteristic-two", "rmk:characteristic-two"],
    );
  });
});
