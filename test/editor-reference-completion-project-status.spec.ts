...
describe("Project-status completion gating (issue #1 Phase 7)", function () {
  ...
  const windowWithIpc = window as Omit<typeof window, "ipc"> & {
  ...
  function optionFor(
    view: EditorView,
    key: string,
  ): { surface: { from: number; options: Completion[] }; option: Completion } {
  ...
  function applyOption(view: EditorView, surface: { from: number }, option: Completion): void {
    assert.strictEqual(
      typeof option.apply,
      "function",
      "label options apply through their handler",
    );
  ...
  it("inserts omitted entries and carries their append-and-continue plan", function () {
    ...
    return waitForAppend().then(() => {
      ...
      const update = recordedInvokes[1].message.payload as {
        path: string;
  ...
  it("leaves citation options byte-identical and affordance-free", function () {
    ...
    const citationOptions = citations.entries(
      ctx,
      citationView.state.doc.sliceString(from as number, pos).toLowerCase(),
    );
    ...
    assert.ok(
      combined.options
        .slice(0, citationOptions.length)
        .every((option) => affordanceOf(option) === undefined),
