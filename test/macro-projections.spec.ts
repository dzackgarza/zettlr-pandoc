...
import {
  projectMathJaxHeader,
  projectTexHeader,
} from "source/app/service-providers/commands/exporter/macro-projections";
...
describe("Exporter macro projections", function () {
  ...
  it("serializes closing script tags safely in MathJax macro bodies", function () {
    assert.strictEqual(
      projectMathJaxHeader({ escaped: "</script>" }),
      [
        "<script>",
        "window.MathJax = {",
        '  "tex": {',
