...
describe('LaTeX/TikZ completion sources', function () {
  ...
  beforeEach(function () {
    ...
    __resetTikzTemplateCompletionCacheForTests()
    ;(window as any).ipc.invoke = async (channel: string) => {
      if (channel === 'tikz-completion-commands') {
        return [
          ...
          { label: '\\includegraphics', argumentCount: 2, declaration: '\\renewcommand{\\includegraphics}[2]{#1#2}' },
        ]
      }
  ...
  it('ranks user macros above stock LaTeX commands sharing their prefix', async function () {
    ...
    const result = await resolveSource(texCommandSource, contextFor(doc, doc.length - 1))
    assert.ok(result !== null)
    const stock = result.options.find(option => option.label === '\\frac' && (option as any).zettlrSource === 'LaTeX')
    const macro = result.options.find(option => option.label === '\\fracId' && (option as any).zettlrSource === 'Macro')
  ...
  it('keeps a user TikZ-template macro visible when it shadows a stock command', async function () {
    ...
    const stock = overlapping.find(option => (option as any).zettlrSource === 'LaTeX')
    const macro = overlapping.find(option => (option as any).zettlrSource === 'Macro')
    assert.ok(stock !== undefined)
    assert.ok(macro !== undefined)
    assert.ok((macro.boost ?? 0) > (stock.boost ?? 0))
  })
  ...
  it('offers tikzcd commands and LaTeX commands together in tikzcd', async function () {
    ...
    assert.ok(arrow?.options.some(option => option.label === '\\arrow'))
    ...
    assert.ok(frac?.options.some(option => option.label === '\\frac'))
  })
  ...
  it('completes ordinary LaTeX environments after \\begin{', async function () {
    ...
    const result = await resolveSource(texCommandSource, contextFor(doc, pos))
    assert.ok(result?.options.some(option => option.label === 'itemize'))
  })
...
describe('LaTeX/TikZ construction snippets', function () {
  it('keeps a broad collision-free mnemonic catalogue', function () {
    ...
    assert.ok(triggers.length >= 60, `expected a broad construction catalogue, got ${triggers.length} triggers`)
    const normalized = triggers.map(trigger => trigger.toLocaleLowerCase())
    assert.equal(new Set(normalized).size, normalized.length, 'construction triggers must be unique case-insensitively')
  })
...
describe('portable prose dictionary completion', function () {
  ...
  beforeEach(function () {
    originalInvoke = window.ipc.invoke
    __resetProseCompletionCacheForTests()
    ;(window as any).ipc.invoke = async (channel: string, message: { command?: string }) => {
      if (channel === 'dictionary-provider' && message.command === 'get-prose-completions') {
        return [ 'therefore', 'theorem', 'on the other hand', 'in particular' ]
      }
      throw new Error(`unexpected IPC ${channel}:${String(message.command)}`)
    }
  })
