...
async function capture(scene) {
  ...
  if (readiness.type !== "object") {
    throw new Error(
      `${scene.name} did not initialize its capture entry: ${JSON.stringify(readiness)}`,
    );
  }
...
async function captureComposited(window, scene) {
  ...
  throw new Error(
    `${scene.name} never composited visible content (every sampled frame was uniform)`,
  );
}
...
app.on("window-all-closed", () => {
  /* keep running until every scene captured */
});
