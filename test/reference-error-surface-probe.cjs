...
app.whenReady().then(async () => {
  ...
  const readiness = await window.webContents.executeJavaScript('typeof window.errorSurfaceProbeRun')
  ...
  const toastsAfterFailure = await window.webContents.executeJavaScript('window.errorSurfaceProbeToastState()')
  const screenshots = []
  screenshots.push(await screenshot(window, 'error-surface-after-failure.png'))

  const dismissPerformed = await window.webContents.executeJavaScript('window.errorSurfaceProbeDismiss()')
  await nextFrame(window)
  const toastsAfterDismiss = await window.webContents.executeJavaScript('window.errorSurfaceProbeToastState()')
  ...
  await nextFrame(window)
  const editorText = await window.webContents.executeJavaScript('window.errorSurfaceProbeEditorText()')
  screenshots.push(await screenshot(window, 'error-surface-typing-after-dismiss.png'))

  const uncaught = await window.webContents.executeJavaScript('window.errorSurfaceProbeUncaught()')
