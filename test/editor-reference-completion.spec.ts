...
const CURRENT_DISPATCH: AutocompletePlugin[] = [
  codeBlocks,
  citations,
  files,
  headings,
...
const COMBINED_DISPATCH: AutocompletePlugin[] = [
  codeBlocks,
  atSymbols,
  files,
  headings,
...
describe("Combined @-symbol completion surface (issue #1 Phase 3)", function () {
  ...
  function applyCompletion(view: EditorView, surface: CompletionSurface, index: number): void {
    ...
    assert.strictEqual(
      typeof option.apply,
      "function",
      "completions on the @ surface apply through their handler",
    );
  ...
  describe("byte-identity differential against the citation provider", function () {
    ...
    const contextMatrix: Array<{
      ...
      {
        name: "a line-start @ with the cursor directly behind it",
        doc: "@",
        anchor: 1,
        appliesToday: true,
      },
      ...
      {
        name: "a bracketed citation behind an authored prefix",
        doc: "[see @Ols",
        appliesToday: true,
      },
      { name: "a query that filters the citation database", doc: "See @kod", appliesToday: true },
      {
        name: "a database sorted by in-document citation usage",
        doc: "We follow [@Kod63]. See @",
        appliesToday: true,
      },
      {
        name: "a line-start @ with a query (no citation context today)",
        doc: "@Ols",
        appliesToday: false,
      },
      {
        name: "an email-like in-word @ (no citation context today)",
        doc: "mail dzackgarza@gmail",
        appliesToday: false,
      },
    ];
    ...
    for (const { name, doc, anchor, appliesToday } of contextMatrix) {
      it(`preserves citation completion byte-identically for ${name}`, function () {
        ...
        assert.strictEqual(
          current !== null,
          appliesToday,
          "the matrix row must reflect the current citation trigger surface",
        );
        ...
        assert.ok(
          ...
          "the citation database must produce options in a citation context",
        );
        ...
        assert.strictEqual(
          combinedView.state.selection.main.anchor,
