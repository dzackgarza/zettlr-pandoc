...
const snapshots: DocumentReferenceSnapshot[] = ALL_PATHS.map((p) =>
  extractReferences(p, sources.get(p) ?? ""),
);
...
describe("previewReferenceRename", function () {
  it("renames a duplicated key across every definition and occurrence with exact ranges", function () {
    ...
    const expectedEdits: WorkspaceTextEdit[] = [
      ...
      // definitions are edited (pinned duplicate-rename semantics).
      {
        ...
        range: tokenRange(THEOREMS_PATH, "#thm:torelli"),
        insert: "#thm:torelli-enriques",
      },
      {
        documentPath: OTHER_PAPER_PATH,
        range: tokenRange(OTHER_PAPER_PATH, "#thm:torelli"),
        insert: "#thm:torelli-enriques",
      },
      ...
      {
        documentPath: HALPHEN_PATH,
        range: tokenRange(HALPHEN_PATH, "@thm:torelli"),
        insert: "@thm:torelli-enriques",
      },
      {
        documentPath: STANDALONE_PATH,
        range: tokenRange(STANDALONE_PATH, "@thm:torelli"),
        insert: "@thm:torelli-enriques",
      },
    ];

    const byPath = (a: WorkspaceTextEdit, b: WorkspaceTextEdit): number =>
  ...
  it("computes inverse undo edits restoring every touched document byte-exactly", function () {
    ...
    for (const documentPath of [THEOREMS_PATH, OTHER_PAPER_PATH, HALPHEN_PATH, STANDALONE_PATH]) {
      ...
      const applied = applyEdits(
        original,
        edit.edits.filter((e) => e.documentPath === documentPath),
      );
      assert.notEqual(
        applied,
        original,
        `${documentPath}: the forward edits must actually change the document`,
      );

      const restored = applyEdits(
        applied,
        edit.undo.filter((e) => e.documentPath === documentPath),
      );
      assert.equal(
        restored,
        original,
  ...
  it("rejects renaming onto an existing key as a collision naming every colliding definition", function () {
    ...
    const collision = previewReferenceRename(
      snapshots,
      "rmk:characteristic-two",
      "rmk:standalone-note",
    );
