...
async function capture (window, scene) {
  ...
  if (scene.expectStatus !== undefined) {
    if (collapsed.projectStatus !== scene.expectStatus) {
      throw new Error(`${scene.name} shows the wrong Project status: ${String(collapsed.projectStatus)}`)
    }
  ...
  if (scene.expand === true) {
    ...
    await screenshot(window, `${scene.name.replace('reference-hover-', 'reference-hover-expanded-')}`)
  }
}
