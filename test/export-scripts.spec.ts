  ...
  it('runs the base Pandoc profile, then the script on the processed output', async function () {
    ...
    const ctx: ExporterAPI = {
      ...
      writeDefaults: async (profileName: string, overrides: Record<string, unknown> = {}) => {
        const profile = YAML.parse(await readFile(`static/defaults/${profileName}`, { encoding: 'utf8' })) as Record<string, unknown>
        return await writeDefaults(profile, overrides, config, [], directory, path.join(directory, 'unused.js'), macros)
      },
      listDefaults: async () => [ { name: 'LaTeX.yaml', reader: 'markdown', writer: 'latex', isInvalid: false } ]
    }
    ...
    const script = { name: 'Marked TeX', profile: 'LaTeX.yaml', command: scriptFile, extension: 'txt' }
