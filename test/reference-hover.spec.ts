...
describe("Reference hover tooltips (issue #1 Phase 4)", function () {
  ...
  it("answers a tooltip spanning exactly the authored @key token of a resolved occurrence", function () {
    ...
    assert.ok(
      tooltip !== null,
      "a resolved supported-family occurrence must produce a hover tooltip",
    );
  ...
  it("presents type, key, path, and enclosing section of the resolved definition", function () {
    ...
    assert.strictEqual(
      dom.querySelector<HTMLElement>("[data-reference-type]")?.textContent,
      "Theorem",
    );
    assert.strictEqual(
      dom.querySelector<HTMLElement>("[data-reference-key]")?.textContent,
      "thm:torelli",
    );
    assert.strictEqual(
      dom.querySelector<HTMLElement>("[data-reference-path]")?.textContent,
      THEOREMS_PATH,
    );
    assert.strictEqual(
      dom.querySelector<HTMLElement>("[data-reference-section]")?.textContent,
      "Main results",
    );
  });
  ...
  it("presents a bounded rendered excerpt of the definition plus an expand indicator", function () {
    ...
    assert.ok(
      text.includes("Two complex Enriques surfaces"),
      "the excerpt renders the definition body",
    );
    assert.ok(!text.includes(":::"), "the excerpt is rendered, never raw fence source");
    assert.ok(
      !text.includes("Halphen pencil of index"),
      "the excerpt is bounded to the hovered definition, never the whole defining document",
    );
    assert.ok(
      !/\bTheorem\s+\d/.test(text),
      "the excerpt never displays a computed reference number",
    );

    assert.ok(
      dom.querySelector("[data-reference-expand]") !== null,
      "the tooltip must present its expand action indicator",
    );
  });
  ...
  it("renders excerpt math with MathJax structure, not flattened Unicode text (issue #7)", async function () {
    ...
    assert.ok(
      container.querySelector("mjx-math") !== null,
      "the container must keep its structural layout elements, not flat glyph text",
    );
  });
