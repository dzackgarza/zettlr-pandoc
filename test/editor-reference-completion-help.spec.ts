...
describe('Completion help link (review A2)', function () {
  ...
  it('every label option carries the quick-help info panel; citation options never do', function () {
    ...
    assert.ok(labels.length >= 10, `the fixture must feed a representative label set (got ${labels.length})`)
  ...
  it('the info panel links to the Pandoc quick reference and the link dispatches the open-help effect', async function () {
    ...
    assert.strictEqual(typeof label.info, 'function', 'the label option must carry an info function')

    const info = await (label.info as (completion: Completion) => Node | { dom: Node } | Promise<Node | { dom: Node }>)(label)
    ...
    assert.strictEqual(observedHelpRequests.length, 0, 'building the panel must not open the help by itself')
    link.click()
    assert.strictEqual(
