...
describe("Reference navigation (issue #1 Phase 5)", function () {
  ...
  const theoremsSource = readFileSync(
    path.join("test", "fixtures", "reference-workspace", THEOREMS_RELATIVE),
    "utf-8",
  );
  const sceneSource = readFileSync(
    path.join("test", "fixtures", "reference-navigation-scene.md"),
    "utf-8",
  );
  ...
  before(async function () {
    ...
    assert.ok(
      jsonLine !== undefined,
      "the navigation probe must return its observed result object",
    );
  ...
  describe("edit-first parity (must stay green)", function () {
    it("ordinary click on a resolved chip reveals the authored source", function () {
      ...
      assert.equal(
        parity.chipPresentBefore,
        true,
        "the resolved @tbl:coble-lattices occurrence must render a chip",
      );
      ...
      assert.equal(
        parity.chipSuppressedAfterClick,
        true,
        "the clicked chip must un-render (preview suppression)",
      );
      assert.equal(
        parity.rawTokenVisibleAfterClick,
        true,
        "the authored @tbl:coble-lattices source must become visible",
      );
    });
    ...
    it("ordinary click on a footnote stays edit-first and never navigates", function () {
      ...
      assert.equal(
        parity.tokenAvailable,
        true,
        "the [^note] token must be visible in the scene document",
      );
      ...
      assert.equal(
        parity.invocations.length,
        0,
        "an ordinary footnote click must not emit any provider request",
      );
    });
  });
  ...
  describe("DocumentLocation capture at jump time (red)", function () {
    it("the probe scene produced a nontrivial fold and scroll to capture", function () {
      ...
      assert.deepEqual(
        location.expected.folds,
        [location.foldRange],
        "the folded paragraph must appear in the live fold set",
      );
      assert.ok(
        location.scrollActual > 0,
        "the scene must genuinely scroll so the capture assertion is discriminating",
      );
      assert.equal(
        location.expected.scrollTop,
        location.scrollActual,
        "the live expectation must read the real scroll offset",
      );
    ...
    it("captureDocumentLocation returns the exact restorable location", function () {
      ...
      assert.ok(
        Number.isInteger(location.captured.sourceGeneration) &&
          location.captured.sourceGeneration >= 0,
  ...
  describe("cross-file Mod-click follow (red)", function () {
    it("resolves the navigation intent for @thm:torelli with the definition’s exact range", function () {
      ...
      assert.equal(
        modClick.rawTokenAvailable,
        true,
        "the authored @thm:torelli token must be reachable for the Mod-click",
      );
      assert.ok(
    ...
    it("emits the documents-provider open-file request with target range and source location", function () {
      ...
      assert.ok(
        ...
        "the request must carry the source DocumentLocation captured at jump time",
      );
      assert.deepEqual(
        {
  ...
  describe("same-file Mod-click follow (red)", function () {
    it("follows @thm:local to its definition inside the same view, without leaving the pane", function () {
      ...
      assert.equal(
        modClick.targetAvailable,
        true,
        "the @thm:local occurrence must be reachable for the Mod-click",
      );
      assert.ok(
        modClick.resolvedIntent !== null,
  ...
  describe("footnote Mod-click follow (red)", function () {
    it("follows [^note] to the footnote body and keeps the jump in the same document", function () {
      ...
      assert.equal(
        modClick.targetAvailable,
        true,
        "the [^note] token must be reachable for the Mod-click",
      );
      assert.ok(
        modClick.resolvedIntent !== null,
