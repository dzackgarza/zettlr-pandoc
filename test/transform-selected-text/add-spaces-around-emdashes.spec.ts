...
describe("MarkdownEditor#addSpacesAroundEmdashes()", function () {
  const sunnyDayTestCases = [
    {
      input: "blush—curious",
      expectedOutput: "blush — curious",
      expectedLengthAfterStripping: 15,
    },
    {
      input: "blush —curious",
      expectedOutput: "blush — curious",
  ...
  sunnyDayTestCases.forEach((testCase) => {
    it(`given "${testCase.input}" ➡️ "${testCase.expectedOutput}"`, function () {
      ...
      addSpacesAroundEmdashes({ state, dispatch });
  ...
  it("given text with emdashes that already have the correct spacing then no transaction is dispatched", function () {
  ...
  it("given text with emdashes that need spacing but the emdash-y bit is not in the selected text then no transaction is dispatched because nothing is selected", function () {
    const state = EditorState.create({
