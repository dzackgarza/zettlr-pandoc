...
describe("Mod-P reference search overlay", function () {
  ...
  before(async function () {
    ...
    await execFileAsync(
      "node",
      [path.join(root, "test/reference-search-overlay-build.cjs"), outputDirectory],
  ...
  it("typing kodemb narrows the workspace definitions to lem:kodaira:embedding", function () {
    assert.equal(
      result.query,
      "kodemb",
      "real keyboard input must land in the autofocused query input",
    );
    assert.equal(
      result.rows.length,
      1,
    ...
    assert.ok(
      result.rows[0].documentPath !== null &&
        result.rows[0].documentPath.endsWith(THEOREMS_RELATIVE),
  ...
  it("headlines every row with the shared display name of its family", function () {
    ...
    assert.ok(
      families.size >= 10,
      `the scene must label many distinct families, got ${families.size}`,
    );
  });
  ...
  it("Enter emits the jump intent for the selected definition", function () {
    ...
    assert.ok(
      result.expectedIntent !== null,
      "the probe must compute the expected intent from real extraction",
    );
  ...
  describe("current-Project ranking and Project markers", function () {
    it("lists every active-Project definition before every other-Project definition", function () {
      ...
      const theoremsCount = extractReferences(theoremsPath, readFileSync(theoremsPath, "utf-8"))
        .definitions.length;
      const otherCount = extractReferences(otherPath, readFileSync(otherPath, "utf-8")).definitions
        .length;
      assert.ok(
        theoremsCount >= 10 && otherCount >= 1,
        "the fixture documents must stay representative",
      );
      ...
      for (const row of rows.slice(0, theoremsCount)) {
        assert.ok(
          row.documentPath !== null &&
            row.documentPath.endsWith(path.join("ProjectA", "Theorems.md")),
      ...
      for (const row of rows.slice(theoremsCount)) {
        assert.ok(
          row.documentPath !== null &&
            row.documentPath.endsWith(path.join("ProjectB", "Other_Paper.md")),
    ...
    it("marks other-Project rows with their Project status and leaves current rows unmarked", function () {
      ...
      assert.ok(
        marked.length >= 1 && unmarked.length >= 1,
        "both groups must be present in the scene",
      );
      ...
      for (const row of marked) {
        assert.equal(
          row.projectStatus,
          "another-project",
          "marked rows carry the computed Project status",
        );
        assert.ok(
  ...
  describe("quick-help affordance", function () {
    it("exposes the [data-open-help] affordance beside the query input", function () {
      assert.equal(
        result.initialState.helpAffordancePresent,
        true,
        "the overlay must expose its quick-help link",
      );
    });
  ...
  describe("badge-keyed reverse lookup", function () {
    it("opening with a key pre-populates the query and enters citing-locations mode", function () {
      assert.equal(
        result.keyed.componentAvailable,
        true,
        `the overlay must mount for the keyed scene: ${result.keyed.componentFailure ?? ""}`,
      );
      assert.equal(
        result.keyed.query,
