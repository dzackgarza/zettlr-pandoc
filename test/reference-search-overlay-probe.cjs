...
app
  ...
  .then(async () => {
    ...
    const readiness = await window.webContents.executeJavaScript(
      "typeof window.referenceSearchProbeMount",
    );
    if (readiness !== "function") {
      throw new Error(
        `reference-search-overlay-entry did not initialize (referenceSearchProbeMount is ${readiness})`,
      );
    }
    ...
    if (mountReport.componentAvailable === true) {
      ...
      // initial state.
      initialState = await window.webContents.executeJavaScript(
      ...
      await nextFrame(window);
      jumpIntents = await window.webContents.executeJavaScript(
        "window.referenceSearchProbeJumpIntents()",
      );
      ...
      const overlayStillPresent = await window.webContents.executeJavaScript(
    ...
    await window.loadFile(pagePath);
    const keyedReadiness = await window.webContents.executeJavaScript(
      "typeof window.referenceSearchProbeMountKeyed",
    );
    if (keyedReadiness !== "function") {
      throw new Error(
        `reference-search-overlay-entry did not initialize the keyed scene (referenceSearchProbeMountKeyed is ${keyedReadiness})`,
      );
    }
    ...
    if (keyedMountReport.componentAvailable === true) {
      ...
      window.webContents.focus();
      await nextFrame(window);
      keyedState = await window.webContents.executeJavaScript(
      ...
      sendKey(window, "Return");
      await nextFrame(window);
      keyedJumpIntents = await window.webContents.executeJavaScript(
        "window.referenceSearchProbeJumpIntents()",
      );
      ...
      const keyedOverlayStillPresent = await window.webContents.executeJavaScript(
        "window.referenceSearchProbeOverlayPresent()",
      );
      if (keyedOverlayStillPresent === true) {
        throw new Error(
          "The keyed overlay must close on the Enter jump before the closed-state frame",
        );
      }
