...
async function computeResponses() {
  ...
  const rawBlocks = SCENE_DOC.split("\n\n").filter((block) =>
    /^\\begin\{(tikzcd|tikzpicture)\}/.test(block),
  );
  ...
  const config = {
    tikzAssetDir: path.join(__dirname, "../static/tikz"),
    cacheDir,
    env: process.env,
  };
  const responses = {};
  ...
  if (
    failures.length !== 1 ||
    failures[0].kind !== "compile-error" ||
    failures[0].errors.length === 0
  ) {
    throw new Error(
      `unexpected render outcomes: ${JSON.stringify(Object.values(responses).map((r) => (r.ok ? "ok" : r.kind)))}`,
    );
  }
  return responses;
}
...
async function capture(window, responses, scene) {
  ...
  if (scene.lightbox === true) {
    ...
    if (shown !== clickedFigure) {
      throw new Error(
        `${scene.name}: the viewer shows ${shown}, not the clicked figure ${clickedFigure}`,
      );
    }
    const lightboxImage = await window.webContents.capturePage();
    await fs.writeFile(
      path.join(outputDirectory, `${scene.name}-lightbox.png`),
      lightboxImage.toPNG(),
    );
