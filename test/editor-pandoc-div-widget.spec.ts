...
describe("Editor presents Pandoc fenced divs semantically", function () {
  ...
  function createEditor(
    doc: string,
    anchor = doc.length,
    extensions: NonNullable<Parameters<typeof EditorState.create>[0]>["extensions"] = [
      markdownParser(),
      renderPandoc,
    ],
  ): EditorView {
...
outside`;
  ...
  function renderedEquations(view: EditorView): string[] {
    return [...view.dom.querySelectorAll<HTMLElement>(".preview-math")].map(
      (element) => element.dataset.equation?.trim() ?? "",
    );
  }

  function activeHybridDiv(view: EditorView): Element {
    const active = view.dom.querySelector(
      'pandoc-div-active-wrapper[data-pandoc-div-family="result"]',
    );
    assert.ok(active !== null, "the theorem shell must be raw while its content is being edited");
    assert.equal(
      view.dom.querySelector("pandoc-div-wrapper"),
      null,
      "the semantic panel must be disabled while editing inside it",
    );
    return active;
  }
  ...
  it("shows an inactive semantic panel while preserving authored content and inner renderers", function () {
    const doc =
      '::: {.layout .definition #term title="Term" style="display:none"}\nA *set* has $\\RR$.\n:::\n\noutside';
    const view = createEditor(doc, doc.length, [
      markdownParser(),
      renderPandoc,
      renderEmphasis,
      renderMath,
    ...
    const opening = view.dom.querySelector(
    ...
    const closing = view.dom.querySelector(
      'pandoc-div-close-wrapper[data-pandoc-div-state="inactive"]',
    );
    ...
    assert.equal(
      ...
      null,
      "unsafe authored styling must not alter the editor layout",
    );
    assert.equal(
      view.dom.querySelectorAll("#term").length,
      0,
      "split presentation wrappers must not duplicate authored DOM IDs",
    );
    assert.ok(closing !== null, "expected a collapsed closing-fence wrapper");
    assert.ok(
      panel?.querySelector(".preview-math mjx-container") !== null,
      "inner math should remain rendered",
    );
  ...
  it("reveals the div shell while preserving cursor-local nested preview behavior", function () {
    const view = createEditor(
      hybridPreviewDoc,
      hybridPreviewDoc.indexOf("Ordinary"),
      hybridPreviewExtensions,
    );
  ...
  it("keeps inline equations rendered while editing ordinary theorem prose", function () {
    ...
    assert.deepStrictEqual(
      renderedEquations(view).filter((equation) => equation !== "z=w"),
      ["x=y", "u=v"],
    );
  });
  ...
  it("keeps display equations rendered while editing ordinary theorem prose", function () {
    ...
    const display = view.dom
      .querySelector<HTMLElement>('.preview-math mjx-container[display="true"]')
      ?.closest<HTMLElement>(".preview-math");
  ...
  it("makes only the selected inline equation raw", function () {
    const view = createEditor(
      hybridPreviewDoc,
      hybridPreviewDoc.indexOf("x=y") + 1,
      hybridPreviewExtensions,
    );
    const active = activeHybridDiv(view);
  ...
  it("makes only the selected display equation raw", function () {
    const view = createEditor(
      hybridPreviewDoc,
      hybridPreviewDoc.indexOf("z=w") + 1,
      hybridPreviewExtensions,
    );
    const active = activeHybridDiv(view);
  ...
  it("makes only selected emphasis raw while links and equations remain rendered", function () {
    const emphasisView = createEditor(
      hybridPreviewDoc,
      hybridPreviewDoc.indexOf("emphasis") + 1,
      hybridPreviewExtensions,
    );
  ...
  it("makes only the selected link raw while emphasis and equations remain rendered", function () {
    const linkView = createEditor(
      hybridPreviewDoc,
      hybridPreviewDoc.indexOf("a link") + 1,
      hybridPreviewExtensions,
    );
  ...
  it("reveals the complete raw div for cursor positions on every authored part", function () {
    ...
    for (const anchor of positions) {
      ...
      const active = view.dom.querySelector(
        'pandoc-div-active-wrapper[data-pandoc-div-state="active"]',
      );
    ...
    assert.ok(
      view.dom.querySelector('pandoc-div-wrapper[data-pandoc-div-state="inactive"]') !== null,
    );
  ...
  it("makes only the deepest selected div active and keeps its container lightweight", function () {
    ...
    const active = view.dom.querySelector(
      'pandoc-div-active-wrapper[data-pandoc-div-family="proof"]',
    );
    const ancestor = view.dom.querySelector(
      'pandoc-div-ancestor-wrapper[data-pandoc-div-family="result"]',
    );
    ...
    assert.ok(
      view.dom.querySelector('pandoc-div-active-wrapper[data-pandoc-div-family="result"]') !== null,
      "selecting the outer fence should activate the outer div",
    );
    assert.equal(view.dom.querySelectorAll("pandoc-div-active-wrapper").length, 1);
  });
  ...
  it("uses the first recognized authored class for each semantic family", function () {
    ...
    const view = createEditor(doc);
    const panels = [
      ...view.dom.querySelectorAll('pandoc-div-open-wrapper[data-pandoc-div-state="inactive"]'),
    ];
  ...
  it("activates the deepest div independently for every selection range", function () {
    ...
    assert.ok(
      view.dom.querySelector('pandoc-div-active-wrapper[data-pandoc-div-family="proof"]') !== null,
    );
  });
  ...
  it("moves the cursor to the source from mouse or keyboard label activation", function () {
    ...
    assert.ok(
      view.dom.querySelector('pandoc-div-active-wrapper[data-pandoc-div-family="definition"]') !==
        null,
    );

    view.dispatch({ selection: { anchor: doc.length } });
    const restoredLabel = view.dom.querySelector(
      'pandoc-div-open-wrapper[data-pandoc-div-state="inactive"]',
    );
