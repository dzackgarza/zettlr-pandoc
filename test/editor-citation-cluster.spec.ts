...
const LIBRARY: Record<string, any> = {
  DM20: {
    ...
    author: [ { family: 'Dolgachev', given: 'Igor' }, { family: 'Markushevich', given: 'Dimitri' } ],
...
describe('Editor parses multi-key citation clusters', function () {
  it('renders every key of a spaced cluster with one separator each', function () {
    ...
    assert.deepEqual(items.map(item => item.id), [ 'DM20', 'DZ99', 'DK13' ])
    assert.equal(
      renderCluster(items),
  ...
  it('keeps a real prefix while dropping the separator whitespace', function () {
    ...
    assert.deepEqual(items.map(item => item.prefix), [ 'see ', 'cf. ' ])
    assert.equal(
      renderCluster(items),
      '(see Dolgachev and Markushevich 2020; cf. Zagier 1999)'
    )
  })
})
