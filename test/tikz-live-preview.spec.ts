...
function success (name: string): TikzRenderResult {
  return { ok: true, html: `<div>${SVG}</div>`, svg: SVG, svgPath: `/cache/${name}.svg`, texFontSizePt: 10 }
}
...
describe('TikZ live preview controller', function () {
  it('trailing-debounces rapid edits to the newest source', async function () {
    ...
    assert.deepStrictEqual(requests.map(request => request.source), [ 'ABC' ])
  ...
  it('keeps one render in flight and replaces queued intermediate source with the newest edit', async function () {
    ...
    controller.setTarget(target('A'))
    await sleep(20)
    assert.deepStrictEqual(requests.map(request => request.source), [ 'A' ])

    controller.setTarget(target('AB'))
    await sleep(12) // AB becomes queued behind A.
    controller.setTarget(target('ABC')) // AB is immediately superseded.
    await sleep(12)
    assert.deepStrictEqual(requests.map(request => request.source), [ 'A' ], 'no second compile starts while A is live')

    first.resolve(success('a'))
    await sleep(5)
    assert.deepStrictEqual(requests.map(request => request.source), [ 'A', 'ABC' ], 'only the newest queued source starts next')
  ...
  it('keeps the last good figure when current source fails to compile', async function () {
    let state: TikzLivePreviewState|undefined
    const controller = new TikzLivePreviewController(
      async request => request.source === 'good'
        ? success('good')
        : {
            ok: false,
            kind: 'compile-error',
    ...
    await sleep(15)
    assert.strictEqual(state?.lastGood?.source, 'good', 'compile failure does not erase the visual reference')
