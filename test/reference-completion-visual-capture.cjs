...
async function capture (window, scene) {
  ...
  if (scene.select !== undefined) {
    const selection = await window.webContents.executeJavaScript(`window.completionProbeSelect(${JSON.stringify(scene.select)})`)
    if (selection.selected !== true) {
      throw new Error(`${scene.name} could not select the ${scene.select} option; walked: ${JSON.stringify(selection.seen)}`)
    }
  ...
  if (!diagnostics.details.includes('Theorem — Torelli for Enriques') ||
    ...
    throw new Error(`${scene.name} is missing the Type — title label details: ${JSON.stringify(diagnostics.details)}`)
  }
  if (scene.select !== undefined && diagnostics.selectedLabel !== scene.select) {
    throw new Error(`${scene.name} shows the wrong selected option: ${String(diagnostics.selectedLabel)}`)
  }
  if (diagnostics.infoText !== null && diagnostics.infoBesideList !== true) {
    throw new Error(`${scene.name}: the info panel overlaps the option list instead of docking beside it`)
  }
  ...
  if (scene.proveInert === true) {
    ...
    if (before !== after) {
      throw new Error(`${scene.name}: applying the disabled another-Project entry changed the document`)
    }
  }
}
