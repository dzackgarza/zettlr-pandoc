...
describe("Pandoc attribute-block rendering (issue #11)", function () {
  ...
  it("renders the heading attribute block away when the cursor is elsewhere", function () {
    ...
    assert.ok(
      ...
      `the attribute block must be rendered away, saw: ${text}`,
    );
    assert.ok(
      text.includes("Central compactification problem"),
      "the heading text itself must stay visible",
    );
  });
  ...
  it("renders multi-entry attribute blocks away on every heading", function () {
    ...
    assert.ok(
      !text.includes("{#sec:two .unnumbered}"),
      `the attribute block must be rendered away, saw: ${text}`,
    );
  });
  ...
  it("renders the table-caption attribute block away", function () {
    ...
    assert.ok(
      !text.includes("{#tbl:coble-lattices}"),
      `the caption attribute block must be rendered away, saw: ${text}`,
    );
    assert.ok(
      text.includes("Coble lattices of Halphen type"),
      "the caption text itself must stay visible",
    );
  });
  ...
  it("reveals the attribute block while the cursor is on the carrier line", function () {
    ...
    assert.ok(
      text.includes("{#sec:central-problem}"),
      `the attribute block must reveal for editing, saw: ${text}`,
    );
  });
  ...
  it("renders the image attribute block away when the cursor is elsewhere (issue #27)", function () {
    ...
    assert.ok(
      !text.includes("{#fig:one .wide}"),
      `the image attribute block must be rendered away, saw: ${text}`,
    );
  ...
  it("reveals the image attribute block while the cursor is on the image line (issue #27)", function () {
    ...
    assert.ok(
      text.includes("{#fig:one .wide}"),
      `the image attribute block must reveal for editing, saw: ${text}`,
    );
  });
  ...
  it("reports an editor assembled without the configuration instead of hiding blocks on a preference nobody set", function () {
    ...
    assert.strictEqual(
      exceptions.length,
      1,
      "the missing configuration is reported through the editor's exception channel",
    );
    assert.ok(
      exceptions[0] instanceof Error,
      `the report carries a real error, got ${String(exceptions[0])}`,
    );
  });
});
