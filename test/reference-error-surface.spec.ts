...
describe("Recoverable reference-error surface", function () {
  ...
  before(async function () {
    ...
    await execFileAsync(
      "node",
      [path.join(root, "test/reference-error-surface-build.cjs"), outputDirectory],
    ...
    assert.ok(
      jsonLine !== undefined,
      "the error-surface probe must return its observed result object",
    );
  ...
  it("the toast dismisses on click and leaves no surface behind", function () {
    assert.equal(
      result.dismissPerformed,
      true,
      "a toast must be present to dismiss (red while no toast surfaces)",
    );
    assert.deepEqual(
  ...
  it("the editor stays interactive: real typing lands in the document after dismissal", function () {
    assert.equal(
      result.dismissPerformed,
      true,
      "the interactivity claim is only meaningful after a real dismissal (red while no toast surfaces)",
    );
    assert.ok(
